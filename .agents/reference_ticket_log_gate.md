...
**How to apply:** Since S0381 Phase 01, `scripts/post-change.ps1` runs this audit with `-Gate` as the `ticket-log-audit` step for ChangeType `Kotlin`/`Mixed`. So any new `Timber.i/w/e("...Sxxxx...")` or non-probe `Timber.d("...Sxxxx...")` makes the next Kotlin post-change FAIL for the whole repo (global invariant, not per-file). When adding logs: never embed a ticket id in permanent log text - reserve `Sxxxx:` for `BlockNeedUserTest` probes only. If post-change fails on `ticket-log-audit`, run the script without `-Gate` to see the offending lines. Related: [[reference_strings_tool]], [[feedback_persistent_logs_no_ticket_id]].
