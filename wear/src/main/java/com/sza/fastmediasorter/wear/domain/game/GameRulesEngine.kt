...
class GameRulesEngine(
    ...
    // Mirrors the phone's applyMove move for move; the shape is held identical on purpose
    ...
    // Mirrors the phone's movePlayer move for move; the shape is held identical on purpose
    // (ADR-1), so the guard chain is suppressed rather than restructured.
    @Suppress("ReturnCount")
    ...
    // Mirrors the phone's pushWall move for move; the shape is held identical on purpose
    // (ADR-1), so the guard chain is suppressed rather than restructured.
    @Suppress("ReturnCount")
    ...
    // Mirrors the phone's moveEnemies move for move; the shape is held identical on purpose
    // (ADR-1), so the guard chain is suppressed rather than restructured.
    @Suppress("ReturnCount")
