...
@Composable
fun WaterFlashlightScreen(onLeave: () -> Unit) {
    KeepDisplayLit()
    ...
    Box(
        modifier = Modifier
            ...
            // Rotation leaves too, for the same reason. On a watch whose bezel is capacitive rather
...
private fun rememberMinuteClock(): String {
    ...
    return remember(now) { DateFormat.getTimeFormat(context).format(now) }
}
