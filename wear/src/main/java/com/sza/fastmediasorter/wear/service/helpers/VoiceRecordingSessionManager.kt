...
class VoiceRecordingSessionManager @Inject constructor(
    ...
    companion object {
         ...
         * what `AacExtractor` on the phone already reads. S3352: uses VOICE_COMMUNICATION audio source
         * to enable Wear OS platform acoustic echo cancellation (AEC) and AGC for live streams.
         */
        fun configureLiveStream(target: MediaRecorder, sink: ParcelFileDescriptor) {
            target.setAudioSource(MediaRecorder.AudioSource.VOICE_COMMUNICATION)
