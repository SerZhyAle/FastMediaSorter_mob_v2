...
class GameScoring(
    ...
    fun afterAcceptedTurn(stats: GameStats, wallPushed: Boolean, shadowsCrushed: Int = 0): GameStats {
        val pushBonus = if (wallPushed) wallPushBonus else 0
        val rawScore = stats.score - turnPenalty + pushBonus + shadowsCrushed * shadowCrushBonus
        val nextScore = rawScore.coerceAtLeast(0)
        return stats.copy(
            turns = stats.turns + 1,
