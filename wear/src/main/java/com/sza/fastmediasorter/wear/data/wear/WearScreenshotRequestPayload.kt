...
/**
 * S3110: the phone asking this watch for a picture of its own screen.
 *
 * The ask carries no payload of substance - it exists so this watch can answer the right request when
 ...
 *
 * Mirrored verbatim from the phone module's copy - `app_v2` does not depend on `:wear`, so the field
 * names pinned here are the whole contract.
 */
...
/**
 * This watch's answer to one screenshot request. Mirrored on the other module.
 *
 ...
 *
 * The two modules share no code, so these strings are the contract; the phone keeps its own copy
...
/** S3110: reads one screenshot request off the wire and writes the answer back onto it. */
object WearScreenshotRequestCodec {
    ...
    fun parseAck(bytes: ByteArray, gson: Gson): WearScreenshotAckParseResult {
        ...
        // A refusal this watch could not correlate carries an empty request id by design, so only a
