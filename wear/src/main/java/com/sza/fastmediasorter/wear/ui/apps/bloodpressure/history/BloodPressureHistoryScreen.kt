...
/**
 * S2809: the blood pressure measurement history screen.
 *
 * Lists all stored measurements newest-first with date, time, and systolic/diastolic values.
 * A clear-all chip at the bottom deletes every entry in one action.
 */
@Composable
fun BloodPressureHistoryScreen(
    ...
    Timber.d("S2809: blood pressure history screen opened")
...
private fun ScreenTitle() {
    Text(
        text = stringResource(R.string.blood_pressure_history_title),
...
@Composable
private fun HistoryRow(entry: BloodPressureHistoryEntry) {
    val dateTime = formatTimestamp(entry.timestampMillis)
    Text(
        text = stringResource(R.string.blood_pressure_history_row, dateTime, entry.systolic, entry.diastolic),
        style = MaterialTheme.typography.body1,
        modifier = Modifier
            .fillMaxWidth()
