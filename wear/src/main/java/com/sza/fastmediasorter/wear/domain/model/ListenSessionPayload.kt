 ...
 * phone module's `WearListenSessionPayload.kt` - the two are hand-kept in step, as the twelve payload
 ...
 * renaming a field here would make the phone's command unreadable rather than merely unknown.
 *
 ...
 * server, whose address arrives in [ListenAckPayload].
 *
 ...
 */
data class ListenCommandPayload(
     ...
     * expired minutes ago is otherwise indistinguishable from the answer to the request in front of
     * the user right now.
     */
 ...
 */
enum class ListenRefusal {

    /** The owner was never asked: notifications are off, so the request could not be posted. */
    ...
    /** The watch is not on a network the phone could reach - strategic pillar E. */
    ...
    /** The phone asked for the session to end, and it has. Not a failure - the answer to a stop. */
 ...
 *
 * A refusal arrives as a value here rather than as an absent ack, so the phone never has to tell
 * "refused" from "lost".
 */
data class ListenAckPayload(
    ...
    /** The port the LAN server bound to. Zero exactly when [refusal] is set. */
    @SerializedName("port") val port: Int,

    /** Null exactly when the watch is serving. */
    @SerializedName("refusal") val refusal: ListenRefusal?
) {

    companion object {

        /** The port the platform never assigns, so it is safe as "no port" on a refusal. */
        private const val NO_PORT = 0

        fun serving(requestId: String, host: String, port: Int): ListenAckPayload =
            ListenAckPayload(requestId = requestId, host = host, port = port, refusal = null)

        fun refused(requestId: String, refusal: ListenRefusal): ListenAckPayload =
            ListenAckPayload(requestId = requestId, host = "", port = NO_PORT, refusal = refusal)
    }
}
 ...
 *
 * A malformed payload decodes to null rather than throwing: a command from a phone on another build
 * must leave this watch exactly as it found it, and an exception crossing `WearableListenerService`
 * would take the process with it.
 */
class ListenSessionPayloadCodec @Inject constructor(private val gson: Gson) {

    fun encodeCommand(payload: ListenCommandPayload): ByteArray =
        gson.toJson(payload).toByteArray(Charsets.UTF_8)

    fun decodeCommand(data: ByteArray): ListenCommandPayload? = try {
        gson.fromJson(data.decodeToString(), ListenCommandPayload::class.java)
    } catch (e: JsonSyntaxException) {
        Timber.w(e, "Undecodable listen command payload")
        null
    }

    fun encodeAck(payload: ListenAckPayload): ByteArray =
        gson.toJson(payload).toByteArray(Charsets.UTF_8)

    fun decodeAck(data: ByteArray): ListenAckPayload? = try {
        gson.fromJson(data.decodeToString(), ListenAckPayload::class.java)?.let(::nameUnknownRefusal)
    } catch (e: JsonSyntaxException) {
    ...
    /**
     * A refusal this build cannot name arrives from Gson as a null [ListenAckPayload.refusal], which
     * is the same shape as "the watch is serving" - so a served address is recognised by carrying a
     * port, and anything else without a reason is named [ListenRefusal.UNKNOWN] rather than played.
     */
    private fun nameUnknownRefusal(ack: ListenAckPayload): ListenAckPayload =
        if (ack.refusal == null && ack.port <= 0) {
            ack.copy(refusal = ListenRefusal.UNKNOWN)
        } else {
            ack
        }
}
