package com.sza.fastmediasorter.wear.ui.icon
 ...
 *
 * Producer: scripts/wear/generate-wear-resource-icons.ps1
 * Source of truth: app_v2/src/main/res/drawable/ico_*.xml
 *
 * S2129: a resource synced from the phone carries its own icon id, and the watch resolves
 * it here. An unknown id answers null so the caller falls back to the type-derived glyph -
 * a resource synced before the id existed must still draw, not disappear.
 *
 * Narrower than the phone's ResourceIconRegistry on purpose: the watch resolves an id and
 * nothing more. The set, ordinal and random helpers there serve the phone's icon picker.
 */
object WearResourceIconRegistry {
    ...
    /** The drawable for [iconId], or null when this build does not carry that icon. */
    @DrawableRes
