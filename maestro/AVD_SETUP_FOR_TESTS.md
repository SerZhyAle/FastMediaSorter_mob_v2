...
## Роль домашнего экрана для launcher-овых сценариев (S2720)

Два flow - `features/launcher/launcher_settings_open.yaml` и `features/launcher/launcher_start_menu.yaml` -
открывают диалог настроек лаунчера, а строка, которая его открывает, активна только пока приложение
удерживает системную роль `android.app.role.HOME`. Оба объявляют это маркером `# maestro-requires: home-role`
в своей шапке.

Что делает раннер `run-tests.ps1` с таким flow:

- Читает текущего владельца роли, запускает flow и **возвращает роль прежнему владельцу** сразу
  после прогона - независимо от того, прошёл flow или упал. Возврат обязателен: пока приложение
  удерживает роль, аварийный выход `_shared/go_home.yaml` следующего flow остановил бы домашнее
  приложение и Maestro не вернулся бы.
- Саму роль берёт не раннер, а flow: `_shared/launcher_mode_enable.yaml` тапает переключатель,
  выбирает кандидата в системном диалоге и подтверждает его. Заранее выдать роль через adb нельзя -
  `cmd role add-role-holder` работает только при уже включённом HOME-компоненте, а `pm enable` для
  чужого компонента шелл отклоняет.
- На эмуляторе разрешение не спрашивается. На физическом устройстве flow **пропускается** со
  статусом `skip` и причиной, пока не передан ключ `-AllowHomeRoleGrant`:

```powershell
pwsh -NoProfile -File maestro/run-tests.ps1 -Suite features\launcher -DeviceId RFCR110NBQJ -AllowHomeRoleGrant
```

Ключ выключен по умолчанию потому, что право выдать системную роль конкретному устройству записано
в `docs/DEVICE_FLEET.md` - сверьтесь с серийным номером там, прежде чем его передавать.

Пропущенный flow не считается дефектом: `pass` набора он не роняет, а `prerelease-verdict.ps1`
печатает его отдельной строкой с причиной.

Роль возвращается сразу, а вот HOME-компонент остаётся включённым до следующего захода приложения
в «Общие» - там `refreshState()` видит, что роли больше нет, и выключает его сам. Поэтому после
launcher-ового прогона шапка раннера может показывать `launcher-mode: ON`: это ожидаемо и на другие
flow не влияет, `_shared/go_home.yaml` такое состояние обходит (S1673).
