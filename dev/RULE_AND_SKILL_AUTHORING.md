...
- **Full digest** - `AGENTS.md`, `.github/copilot-instructions.md`. Read by agents that never load the authority (Copilot pulls its file into every chat and pulls no other), so each must name **every** numbered rule and cite it as the literal token `Rule N`. Its own numbering, wording and grouping are free; only the citation is fixed, because the citation is what a machine can check. A range (`Rules 24-29`) does not count as citing its members - that is precisely how rules 25-28 looked covered while nothing stated them (S1548). A full digest owes a **second citation unit** beside `Rule N`: every closing gate carrying an `Sxxxx` label in the `## Closing gates` section of `.claude/rules/spec-catalog.md` must be stated and cited by that literal token. Those gates are numbered by ticket rather than by rule, so they sat outside the `Rule N` unit and their omission was silent until S2583 - the copilot digest cited none of the six. The unit is the bullet's leading bold label, never its prose: that prose names neighbours and incidents, and requiring those cited would demand a retelling of other people's tickets. A bullet with no label token is out of reach by construction (currently one: the dated owner ruling `A found problem ends BlockNeedUserTest`, which owns no ticket id), and a bullet that later gains a label enters the check on its own.
...
So: adding rule 31 means writing it in `CLAUDE.md` and stating it in both full digests with a `Rule 31` citation; adding a closing gate `S9999` means writing it in `.claude/rules/spec-catalog.md` with its id inside the bullet's bold label, and stating it in both full digests with an `S9999` citation. Nothing else is owed.
