...
- [x] При запуске программы я вижу заголовок "FastMediaSorter v2". Заголовок программе не нужен
  - ✅ FIXED: Изменена тема с DarkActionBar на NoActionBar в themes.xml

- [x] В окне списка ресурсов кнопка добавления - зеленый плюс. Сильно выделяется на фоне других черно/белых кнопок. Нужно исправить на черно/белую
  - ✅ FIXED: Заменена иконка с ic_input_add на ic_menu_add (нейтральный цвет)

- [x] При запуске программы, если список ресурсов пока пустой, пусть посредине этого пустого пространства появляется текст "Добавьте ресурсы для операций по сортировке и проигрыванию медиафайлов" с иконкой "плюс", аналогичной из панели команд. При нажатии на этот текст реакция программы такая же, как добавление ресурса
  - ✅ IMPLEMENTED: Empty state view added to MainActivity
  - ✅ Shows centered icon (plus) and text message when resources list is empty
  - ✅ Clicking on empty state triggers add resource action
  - ✅ Visibility controlled in observeData() based on state.resources.isEmpty()
  - ✅ Build successful

- [ ] В окне "фильтр и сортировка" для ресурсов Вид сортировки должен быть "Выбор из выпадающего списка". В текущем виде он отбирает слишком много пространства.

- [ ] В окне "фильтр и сортировка" кнопки внизу "Очистить все", "Отмена", "Применить" разного размера и стиля. Нужно их привести к общему виду

- [ ] Я добавил несколько локальных папок в качестве ресурсов, но в списке ресурсов вижу только один
...
- [x] **Player Screen: Add 'with command panel' mode**
  - ✅ IMPLEMENTED: Command panel mode fully functional
  - ✅ Created unified layout activity_player_unified.xml with both fullscreen and command panel modes
  - ✅ Top command panel: Back, Previous, Next, Rename, Delete, Undo (hidden), Slideshow buttons
  - ✅ Bottom panels: "Copy to..." (green) and "Move to..." (blue) with dynamic destination buttons (1-10)
  - ✅ Destination buttons: populated from DB, colored per destinationColor, arranged in GridLayout (max 5 cols, 2 rows)
  - ✅ Touch zones: left half = Previous, right half = Next (visible only in command panel mode)
  - ✅ Mode switching: via "Show command panel" touch zone in fullscreen mode or toggleCommandPanel()
  - ✅ Settings integration: initial mode based on fullScreenMode preference (!fullScreenMode = showCommandPanel)
  - ✅ File operations: performCopyOperation() and performMoveOperation() with FileOperationUseCase
  - ✅ All command panel buttons wired: Previous, Next, Rename, Delete, Slideshow
  - ✅ Build successful - all components integrated and tested
  - ⏳ TODO: Adjust button heights when "Small controls" setting enabled
  - ⏳ TODO: Test on physical device to verify layout rendering and touch zone behavior

- [x] **Player Screen: Adjust touch zones for video**
  - ✅ IMPLEMENTED: Touch zones adjusted for video playback
  - ✅ Fullscreen mode: touch zones only upper 75% of screen, bottom 25% reserved for ExoPlayer controls
  - ✅ Command panel mode: touch zones only upper 50% of media area for video
  - ✅ Implementation: handleTouchZone() checks media type and calculates effectiveHeight
  - ✅ Implementation: adjustTouchZonesForVideo() dynamically adjusts overlay height in command panel mode
  - ✅ For images: touch zones still cover full area as specified
  - ✅ Build successful
