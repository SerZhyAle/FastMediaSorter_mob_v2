...
Ключевая идея v2 - объединить просмотр, воспроизведение и организацию файлов в одном интуитивно понятном интерфейсе, устраняя недостатки и ограничения предыдущей версии. Не будем притворяться, что v1 была безупречной - именно поэтому и появилась v2.

## Что заменяет FastMediaSorter 🧩

FastMediaSorter - это универсальный медиабраузер, просмотрщик, плеер и органайзер. Одно приложение покрывает то, для чего обычно нужен десяток отдельных программ. Известные приложения ниже приведены лишь как узнаваемые ориентиры для каждой возможности, а не как сравнение.
...
- **OCR и Перевод**: Tesseract4Android (извлечение текста для всех письменностей), Google ML Kit (перевод и определение языка)
