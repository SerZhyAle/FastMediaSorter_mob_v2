...
- Crop (overwrite original), Crop to file (save fragment as new file), and Compressed copy (JPG 70%, long side ≤ 1024 px) commands in the overflow menu; work on local and network sources (SMB, SFTP, cloud); read-only sources auto-redirect to Downloads. Output files for Crop to file and Compressed copy are named `<original>_crop-YYMMDD-hhmm.<ext>` and `<original>_compress-YYMMDD-hhmm.<ext>` respectively.
- **Draw annotations**: Activate "Draw" from the image overflow menu to enter draw mode. Select from a brush, rectangle, or eraser tool with a 7-color palette (white, black, gray, red, blue, green, yellow). Tap "Save as new file" to merge annotations onto the image and save it as a new file (original untouched). The new file is named `<original>_draw-YYMMDD-hhmm.<ext>` and the player navigates to it automatically. For read-only sources, the file is saved to the device Downloads folder.
...
- **Adaptive widget polish**: Widgets adapt to launcher theme on Android 12+ (Material You) and show distinct names and chip icons in the system picker; Favorites widget displays a hint when no favorites are saved yet.
...
- **Onboarding walkthrough**: Each first-run Welcome page shows a compact header (icon · title · summary) with the full multi-paragraph explanation available by scrolling down; a dedicated overview page presents the app's extra tools as an adaptive grid of small blocks (column count adapts to screen width and orientation, BuildConfig-gated per flavor); the touch-zones page draws the 3×3 action map inline so it stays crisp at any size. Navigation is one fixed bottom bar — Back · page indicator · Skip · Next — where Skip jumps to the final onboarding page (the "set as default player" offer), or to the media-access step on flavors without it, rather than exiting the flow. Adapts to portrait and landscape on phones, tablets and TV.
...
- **Permission Management:** On first launch a full list of requested permissions is shown with descriptions. Re-accessible any time via Settings → Manage Permissions. Context-triggered requests appear when enabling features that require specific permissions.
...
- **Behaviour group sub-sections**: Inside Playback → Behaviour, the link auto-download options (auto-download incoming links, download destination resource, open downloaded file in player, saved authorizations) and the camera-capture options (disable camera capture button, skip the filename prompt) are now grouped into divider-delimited, labelled sub-sections ("Incoming links" / "Camera capture") so they no longer blend with the surrounding switches. "Saved authorizations" is a clickable row that opens the in-app web sign-in management screen and carries an inline help tooltip. Applies in both portrait and landscape.
...
- **Accept shared files enabled by default (S0133)**: On a fresh install the "Accept shared files" toggle is ON by default — after the first app launch the app appears in the OS share-sheet for the MIME types supported by the current flavor. Disabling remains available in `Settings → Playback → Default Media Player → Accept shared files`. An idempotent bootstrap on every process start re-applies the saved value, so the registration is restored automatically after a reinstall or clear-data.
...
- **Structured link hints and safer shared-link auth (S0140)**: Standard web pages now contribute media URLs through JSON-LD and oEmbed hints before generic HTML scanning runs. A shared text payload may contain multiple http(s) links — the app de-duplicates them and downloads them as one batch. Pages that expose media only after JavaScript execution now get one extra hidden WebView pass before the download gives up. Obvious login walls are surfaced as sign-in-required instead of a generic no-media failure. The WebView authorization dialog now shows a sign-in hint, keeps **Save authorization** disabled until cookies appear, and silently prunes empty saved sessions instead of storing unusable entries.
- **Social-link authorizations (S0144)**: Sharing a link from a known social network (Instagram, Pinterest, TikTok, X, DeviantArt, Threads, Reddit, Tumblr, Flickr, ArtStation) offers to add an authorization for it before the download starts. The "Saved authorizations" screen now has a top toolbar with a "+" action; adding an authorization lets you pick a resource from the built-in list or enter the address manually. Signing in to Instagram and similar resources through the in-app browser no longer fails with an "unknown URL scheme" error.
