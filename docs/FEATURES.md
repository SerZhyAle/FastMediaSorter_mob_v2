...
- **Resource menu actions** `[Standard / Lite / Photos / Legacy / VR]`: The dropdown menu on each resource tile starts with an **Open** action (same as tapping the tile body) and, for media libraries (video, audio, photo storage), a **Launch** action that begins playback at once; **Move to very top** and **Move to very bottom** send a resource to either end of the list in a single step alongside the existing step-by-step move. Media-library tiles also draw a permanent frame around their tappable icon, signalling that the icon launches playback separately from the rest of the tile.
...
- **Record video to resource** `[Standard / VR]`: A dedicated "Record video" command in the resource menu records a clip with the system camera straight into the current folder (local, network, or cloud) for any video-capable resource. Settings let you enable the command, choose a fallback destination resource for cases where the current resource can't be used, and optionally open each recording in the player after it is saved (off by default). A usable current resource remains primary; otherwise the selected destination is used, and if both are unavailable the recording falls back to the device Movies folder. Recordings never open the drawing editor.
...
- **Resource shortcuts** `[Standard Only]`: Single-tap home screen widgets to open a specific saved resource immediately.
...
- **Quick capture widget** `[Standard / Lite / Photos / Legacy / VR]`: Icon-style 1×1 home-screen widget that opens the camera on tap and saves the photo or video straight to a target you pick when adding it - a writable resource (local, NAS, or cloud) or the device camera folder. When adding it you also choose photo or video capture mode; the first use asks for camera access, and an unconfigured or disabled widget prompts you to set its target.
