...
## Wear OS Resource Icons

The Wear OS watch module mirrors the phone's `ico-NN-NNN` vector drawables into `ContentTypeCatalog` so that one entity presents the same glyph across both apps. This mirror is generated by `scripts/wear/generate-wear-resource-icons.ps1` and must not be edited by hand. The parity gate (`scripts/quality/assert-resource-icon-parity.ps1`) checks that the two copies do not drift.
