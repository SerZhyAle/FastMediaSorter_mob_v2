...
- **One release, several archive directories (S2722).** The archive is keyed by each ARTIFACT's own `versionCode`, and since S2721 the watch's code is not a function of the phone's, so a release that ships both modules occupies two directories with two manifests. The phone and the watch stamp the same `versionName`, so that is what identifies a release across the archive: the gate resolves the judged release by name and verifies every payload found under it, in either directory. Before this it derived one code with the `app_v2` formula and opened that directory alone, so the watch mapping was never judged by anything and the PASS line naming `standard` read as "looked and found nothing" when it meant "did not look". A missing wear payload is now stated in the verdict (and as `wearRetained` in `-Json`) rather than failed, because the archive cannot distinguish a release that published no watch artifact from one whose watch retention was lost.
