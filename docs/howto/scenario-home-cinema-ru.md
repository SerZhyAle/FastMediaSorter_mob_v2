...
{% include lang-switcher.html doc="scenario-home-cinema" dir="/docs/howto/" current="ru" %}
...
1. Откройте приложение → **Добавить <img src="../icons/doc/ic_add.png" alt="" width="18" height="18" style="vertical-align:text-bottom">** → **"Сетевая папка (SMB)"**
...
Затем запустите слайдшоу в плеере (команда **Слайдшоу**) и включите **Настройки → Плеер → Проигрывать видео/аудио в слайдшоу до конца** - тогда каждая серия доиграет до конца, прежде чем начнётся следующая.
...
- Используйте **Избранное** (нажмите <img src="../icons/doc/ic_star_filled.png" alt="" width="18" height="18" style="vertical-align:text-bottom"> во время просмотра) для закладки текущего сериала
...
| Видео не воспроизводится (ошибка формата) | В плеере нажмите **параметры <img src="../icons/doc/ic_more_vert.png" alt="" width="18" height="18" style="vertical-align:text-bottom">** → переключите **Декодер** с «Аппаратного» на «Программный» (медленнее, но совместимее) |
| Эпизоды идут в неправильном порядке | В настройках папки (Изменить) установите сортировку **По имени (А→Я)** |
| Авто-переход не работает | Убедитесь, что слайдшоу запущено и Настройки → Плеер → Проигрывать видео/аудио в слайдшоу до конца включено |
