...
- **Отправить каналы с телефона.** Канал, который вы добавили сами в FastMediaSorter на телефоне, отправляется командой **Send to watch** из списка трансляций. Каналы, закреплённые на телефоне, к тому же поднимаются к началу списка на часах - следом за теми, которые вы закрепили на самих часах, - и те два-три, которые вы действительно смотрите, оказываются доступны без прокрутки. Если снять закрепление на телефоне, канал уйдёт из этой группы, а канал, которого нет в каталоге часов, просто пропускается.
...
- **Фильтр** показывает и кураторские подборки, пришедшие вместе с каталогом, - «TV России», «Радиостанции бывшего СССР», «Телевидение Африки» и остальные, те же самые, что и на телефоне. Выберите подборку, чтобы увидеть только её каналы, или **«Все»**, чтобы снять ограничение. Канал может входить сразу в несколько подборок, поэтому одна и та же станция встретится не в одной. Если в скачанном каталоге подборок нет, пункт не показывается вовсе.
...
5. **Погасить экран:** в меню плеера есть команда **Погасить экран**. Экран становится полностью чёрным - ни часов, ни панели управления, - канал продолжает играть, а часы не засыпают. Одиночное касание лишь отмечает точку белым кружком; картинку и панель управления в том же виде, в каком вы их оставили, возвращают двойное касание, долгое нажатие или кнопка самих часов.
6. **Закрепление:** отметка в плеере закрепляет канал. Закреплённые каналы идут первыми при следующем открытии Streams: сначала закреплённые здесь, на часах, за ними - закреплённые на телефоне, а остальные сохраняют порядок выбранной сортировки. Отметка привязана к адресу канала, поэтому переживает повторный импорт каталога.
...
- Если текущее соединение не тянет трансляцию, часы сообщают об этом, а не молча обрываются.
- Если трансляция замирает без ошибки - обычный способ смерти живого эфира, - сторож перезапускает её до трёх раз, показывая **Reconnecting**. И только если сеть так и не ожила, появляется сообщение о недоступности канала.
...
| Канал открывается и сразу останавливается | Возможно, недоступен сам источник. Часы пробуют три раза, прежде чем сдаться - откройте другой канал, чтобы отличить мёртвую трансляцию от мёртвой сети |
| Видео останавливается, когда опускаешь руку | Так и должно быть: после ухода приложения с экрана в фоне продолжается только звук. Чтобы канал играл при тёмном экране, останьтесь в плеере и выберите в его меню **Погасить экран** |
