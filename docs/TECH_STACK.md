...
- `wear` minSdk: `28`
- `compileSdk` / `targetSdk`: `35` for both modules
- NDK: `27.2.12479018` (r27c) in `app_v2`
- `coreLibraryDesugaring` is enabled in `app_v2` for API 23-25 support
- Annotation processing split: `app_v2` uses `kapt`, `wear` uses `ksp`
...
- Sideload / XR-only surface: Chaquopy Python `3.12`, `yt-dlp` nightly `2026.07.21.234255`, OpenXR loader `1.1.48`, NewPipeExtractor `v0.26.1`
...
- SFTP: JSch (`com.github.mwiede:jsch`) in both modules, with different pinned versions
