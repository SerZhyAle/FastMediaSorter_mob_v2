...
- Exempt by design (do not migrate to this family): player/media `ImageButton` borderless controls, reserved ExoPlayer `@id/exo_*` controls, and the intentionally dark camera/viewfinder surfaces. Exempt from **this** family, not from having a named style: since S3250 the player overlay icons carry `Widget.FastMediaSorter.Button.Icon.Overlay`, which is declared `parent=""` precisely because it dresses an `ImageButton` and must not inherit the `MaterialButton` background contract.
...
Every operation that leaves the process - a network round trip, a file write, a cloud request - obeys one contract, and the contract is a property of the seam rather than of any one protocol. The seam is the **transfer strategy layer** under `data/transfer/`: the `FileOperationStrategy` interface, its per-protocol adapters (`LocalOperationStrategy`, `SafOperationStrategy`, `SmbOperationStrategy`, `SftpOperationStrategy`, `FtpOperationStrategy`, `CloudOperationStrategy`), the cross-protocol pairs under `data/transfer/strategies/`, and the byte-level `FileAccess` adapters under `data/transfer/access/`. SMB, FTP, SFTP, cloud and local are five instances of one rule; an adapter family added later joins the same rule rather than negotiating its own.

- **Bounded in time.** Every call declares its own timeout - connect, read and whole-operation - and no call inherits "whatever the library defaults to". A budget that expires surfaces as `NetworkTimeoutException`; a whole-scan watchdog surfaces as `ScanTimeoutException`, which is deliberately a separate type because it reports the backstop firing rather than one socket giving up. An operation with no declared bound is the defect: the user sees a spinner that never resolves and no log line says why.
- **Cooperatively cancellable.** Strategy methods are `suspend` functions and cancellation travels on the coroutine, so the caller cancels a job and never kills a thread. A long walk checks the job between units of work, not only at the boundaries: `DirectoryTreeTransferManager` and the directory operations check it per entry through the progress callback, so a cancelled transfer stops at the entry in flight. Progress reporting is part of the contract, not decoration - `ByteProgressCallback` and `TransferProgressReporter` are how the surface learns the operation stopped.
- **Classified, never raw.** A failure that crosses the seam is one of the `NetworkException` subtypes declared in `data/network/exceptions/NetworkExceptions.kt` - `NetworkTimeoutException`, `NetworkAccessDeniedException`, `NetworkFileNotFoundException`, `NetworkConnectionLostException`, `NetworkServerErrorException`, `NetworkRateLimitException`, `NetworkUnsupportedOperationException`, `NetworkHostKeyChangedException`, `LocalNetworkPermissionDeniedException`, `WifiRequiredException`, `ScanTimeoutException`. `NetworkErrorClassifier.classify` is the one mapper; an adapter that invents a parallel taxonomy, or lets a raw `IOException` reach a ViewModel, breaks the message table (`NetworkErrorMessageMapper`, `BrowseFriendlyErrorResolver`) that turns the type into what the user reads.
- **Retry only when the repeat is safe.** Retry is driven by `RetryPolicy` / `withRetry`, and what may be retried is decided by classification, not by the call site's optimism: `NetworkErrorClassifier.isTransient` admits timeout, connection-lost, rate-limit and 5xx, and nothing else. An access denial, a missing file, a changed host key and an unsupported operation are permanent by construction and are never re-attempted - a host-key change is a security event, and retrying it would turn a warning into a loop. A retry loop written by hand, outside this seam, is the defect.
- **Idempotent where a repeat is possible.** A retried or resumed operation must leave the same end state as one that ran once. A write lands through a temp file and a single publishing step (`AtomicFileOperationStrategy`, `TempFileNamingStrategy`, `LocalSink.commit` / `LocalSink.abort`), so a partial byte stream is never visible under the destination name and an aborted attempt leaves nothing to clean up by hand. A move deletes its source only after that entry's copy is confirmed, which makes a repeated move idempotent rather than destructive. An operation that cannot be made idempotent declares itself non-retryable instead of retrying and hoping.
- **Partial state is named, not silent.** Where a repeat is not possible - a multi-entry folder transfer - the guarantee is stated rather than implied: already-written entries stay at the destination, the message says so, and folder transfers have no undo (S1326). A per-file destructive operation registers in the mutation journal (`domain/mutation/MutationJournal.kt`) through the **caller that owns the resource**, never through this seam: every journal entry is keyed by a resource id, and a strategy here addresses a path and credentials and has no resource concept at all. The four producers are the Player, the local `FileObserver`, the scheduler and the watch bridge, and each registers from the layer that already holds the id - `domain/mutation/MutationRecorder.kt` is the shared helper (S3376 ADR-1). What the journal buys is **list reconciliation**: Browse folds pending entries into its cached list on resume, which is what a background actor needs, because nothing redraws on its behalf. **Undo is a separate and local-only capability** - `domain/usecase/DeletePathPolicy.kt` refuses soft delete for every `content://`, `smb://`, `sftp://`, `ftp://` and `cloud://` path, so no `.trash` snapshot is ever taken for a remote transport and a delete there is irreversible; the journal reconciles it, and reconciling is all it can do. `scripts/quality/assert-fileop-journal-pairing.ps1` refuses a new destructive operation in these trees that names neither its registering caller nor the read-only allowlist; the 59 pre-existing rows are a named structural exclusion rather than a queue of work. The caller half is `scripts/quality/assert-mutation-producer-registration.ps1` (S3386), and its subject is narrower than "every caller that deletes": a caller under `ui/` belongs to a surface that redraws itself, so the gate judges only a **surfaceless producer** - a class the framework starts outside `ui/`, a Worker, a Service, a receiver, a widget provider or a `FileObserver`, that changes user content directly or through one declared collaborator. Such a producer records a `Mutation` or carries a row in `scripts/quality/mutation-producer-registry.txt` naming the compensating refresh that redraws instead (resolved in source, so an unwired one is refused) or declaring that it removes only app-owned state. Measured 2026-09-22 the live set is eight, of which one journals - the registry is that set fully declared, not a frozen baseline.
...
- **Contract ownership**: the schema is `FMSCFG`, owned by the companion and living in the shared contracts catalog - see `docs/CROSS_PROJECT_CONTRACTS.md`. This repo is authoritative for the **consumer** half only, and restates none of the contract here: not the field list, not the transports, not the version-bump rule, because a second copy drifts and this section already held one. The canonical vectors are frozen on both ends (`CompanionConfigParserTest`, and the fixtures under `app_v2/src/test/resources/companion/`, which are byte-identical to the catalog's). Producer-side work lives in the external repo (see S0421, `BlockExternal`).
- **What the consumer half owes**: refuse a `schemaVersion` above `CompanionConfigParser.SUPPORTED_SCHEMA_VERSION` outright rather than parsing best-effort, and surface it as "update the app"; ignore an unknown `accessPaths[].kind` and an unknown `profile` token rather than failing; keep the per-root defaults equal to the ones the contract's table names.
...
- **Validation invariants** (consumer-owned): empty password / empty host-key fingerprint are legal Android-side (password typed at import; a resource imported with no fingerprint connects unpinned - nothing captures a key on first connect, S3466 carries the consent pin; a stored fingerprint is enforced on every session through `SftpHostKeyPinRegistry`) even though the producer always sends both. The rest - which `protocol` is accepted, how `accessPaths` is ordered and that every entry is tried rather than only the first - is the contract's, not ours to restate.
...
**First-run starter set.** An empty desktop is seeded once from `LauncherStarterSets`, the table for what a detected device profile receives. Third-party app cells are conditional on the matching package being installed, so a first seed never leaves a dead app icon; existing desktops are not rewritten when the profile changes. **A feature cell carries the same kind of condition (S2382):** it is seeded only when the feature is launchable - compiled into the build *and* switched on - rather than merely compiled in, so a capability the user has turned off is absent from the desktop it never asked for. Nothing is lost by the omission: the reactive shortcut sync places the cell the moment the feature becomes launchable, which is also why the earlier rule of seeding a disabled feature's cell as a route to its own setting no longer buys anything. The set is packed **per section, not across the whole grid**: a section header raises a packing floor to its own row, and nothing seeded after it may anchor above that row. The floor is a correctness rule before it is an aesthetic one, because section membership is positional - a cell that backfilled the gap a shorter group left behind would belong to the section above it and collapse with it. Content leads the set and the launcher's own actions close it, so the first screen of a phone carries the media resources rather than five service shortcuts; the actions stay reachable from the Start menu, which is what makes that order safe. **One cell per profile carries the signature (S2385).** The profile's widget group opens with the single cell that says what device this desktop belongs to - the live map on a head unit, YouTube on a tablet - ahead of everything else that group earns, because the group is budgeted and a signature seeded at the tail is the first thing a compact screen cuts. The mapping is exhaustive over the profile enum, so a new profile names its signature or an explicit none rather than inheriting a silent default, and nine of the eleven name none because their group is already unlike any other. A signature that is an application joins the seed's package-uniqueness contract as a third owner ahead of both app sections, so it is never the same icon twice on one desktop. **Composition has two axes (S2309).** The profile decides intent - which groups have anything in them at all - and the device's screen class decides capacity: the section order, how many items each section seeds, and how many screens the desktop fills, which is composed rather than fixed at two. The class is a discrete pair - size by smallest width in dp, shape by the long-to-short ratio - derived by `LauncherScreenClassifier` from the running device and independent of the current orientation, so rotating a phone cannot change what its desktop contains. The rules live in `LauncherStarterLayoutRules` and take the screen class alone: a layout stated per profile was measured wrong, because a 20:9 phone and a 4:3 tablet can carry one profile and want different first screens. Several groups may seed one section key - the profile gadgets beside the utility widgets, the core resource aggregates beside the user tail beside the media windows - and when they land on one screen they are emitted under a single header, because a section is addressed by its key alone and two headers sharing one would fold together and collide in the packing pass; the screen cut is pushed past any such neighbouring pair, since two halves of one key on two screens would fold into each other. **A per-section budget applies only where a shorter list is still a list (S2321).** The content aggregates - recent, the four media types, the camera and "All files" - are a closed set that cannot grow without an edit to `LauncherStarterSets`, and each entry is the desktop's only way into a whole content type, so they seed under their own unbounded `CORE_RESOURCES` group while the open user tail keeps the budget; the launcher's own actions are exempt for the same reason. A budget reaching either group removes a capability rather than trimming a list - which is exactly what a compact screen did to "All documents", the camera and "All files" until they were split out. **A content type switched on after the seed reaches the desktop by the same reactive route (S2564).** The aggregates seed from the virtual resources that exist at seed time, and the resource table self-heals - provisioning is idempotent and re-reads the settings - but the seed's already-seeded early exit sits above that call, so a seeded desktop re-reads neither. The resource-tile sync mirrors the shortcut one for the other half of the desktop: it keeps its own baseline of virtual paths, adopts silently on its first pass, places only what became present since, and writes the baseline as a union, so a tile the user deleted never returns. What decides placement is the resource table and never the media-type flag - a tile needs the id of a record that exists, and the composite condition that creates an aggregate lives in provisioning, where a second copy would be the hand-listing the shortcut sync already removed. **Every gadget key declares whether the seed uses it (S2672).** `LauncherGadgetSeedPolicy` in `src/main` is the one home of the gadget key literals that table emits, and it answers for the whole registry rather than only for the keys it seeds: each key is `Seeded` or carries the reason it is not - a widget instance only its configuration screen can mint, a param only a person can choose, an owner ruling, or an owner decision not yet taken. The distinction matters because the registry lives in `src/launcherEnabled` and the seed cannot import it, so before this the two lists had no common home and twenty keys had drifted out of every profile with no recorded decision. `LauncherStarterSetsParityTest` reads the registry's keys by reflection, so a new gadget fails the build until it declares a decision, and it proves each `Seeded` key by running `itemsFor` across every profile rather than trusting the label.

**Grid.** The desktop is a hand-written `ViewGroup`, deliberately not a `RecyclerView` (ADR-9): the persisted model is a canvas with 2D positions, spans and meaningful gaps, which no stock `LayoutManager` expresses - and a desktop is dozens of cells, not a feed, so a `RecyclerView`'s machinery costs the model more than it returns. What that never meant is that re-inflating the cells is free: `LauncherCellViewBinder` hands the torn-down render's roots to the next one as blanks, and on a 36-cell desktop that took the four-rotation `gfxinfo` p99 from 150 ms to 101 ms (S2686, Galaxy S21+). The pool is not a step back toward diffing - no list is compared, no subtree is updated in place, and the tree is still rebuilt whole; a pooled root is re-bound in full, listeners included, which is what keeps it clear of the stale-bind problem that killed the `RecyclerView` renderer. It is also refused across an edit-mode boundary, because edit decorations are added as children and never removed. Column count resolves from available width and a user density factor within a fixed range; height is the scroll axis. All footprint arithmetic funnels through one geometry helper precisely so layout, hit-testing and the free-slot sweep cannot disagree. Drag-to-move uses a container-level `OnDragListener` with `startDragAndDrop` rather than `ItemTouchHelper`, which is RecyclerView-only for the same ADR-9 reason.
...
The cross-project icon contract (`ICON-RENDER` 0.10, section 10) gives one glyph three looks - mono, colour, decorated - and says which surface takes which: an app shortcut and an in-app launch grid take the decorated look, a row icon that states a type takes the colour look, everything else stays mono. The product implements it with four roles, none of which owns a colour:

- **Hue.** `IconHueCatalog` (`core/panel/`) answers "which hue does this meaning wear" by delegating: a program to `SubProgramAccentCatalog`, a resource type to the `color_source_*` family, anything without a tone to `color_icon_accent`. So a program wears one tone on every entrance - its panel row, its launch tile, its home-screen shortcut.
- **On-plate colour.** `PlateContrast` (`core/icon/`) is the contract's rule in pure Kotlin: white wherever white reaches 3:1 against the plate, near-black otherwise.
- **In-app plate.** `GlyphPlateDrawable` (`ui/icon/`) wraps the mono glyph unchanged on a circle of its hue, the glyph at 0.6 of the circle. A custom Drawable ignores a view tint, so a recycled view cannot recolour a plate.
- **Launcher plate.** `DecoratedShortcutIcons` (`core/icon/`) renders an adaptive icon on API 26+ (full-bleed background, 44 dp glyph) and a 44 dp circle on API 25, always in the day tone because the launcher, not the app theme, draws it. The three static shortcuts use the same shape as XML (`ic_shortcut_*`), with day-only plate colours for the same reason.

**The rule that holds it together: a look is derived, never drawn.** A coloured or painted copy of a glyph is what the product had before - the gold note, the blue network folder - and what the contract forbids. The measured glyph style and the size tiers are enforced by `scripts/quality/assert-icon-style.ps1`; `docs/icons/README.md` "Looks and the style gates" is the working reference.
...
- **Orchestrator:** `CollapsibleSectionsManager.register(header, container, key, defaultExpanded, persistState, onExpandedChanged?)` binds a header to its content container, animates the body open/close, announces expanded/collapsed for TalkBack, and persists state unless `persistState = false`. The optional `onExpandedChanged` hook supports lazy first-expand work (e.g. attaching a child fragment on first expand).
...
- **A one-off dialog does not persist its sections at all (S3355).** `defaultExpanded = true` is not enough there: the stored value always wins over the default, so one collapse reopens the dialog folded away for good - the folder picker showed no folder at all in that state. A dialog passes `persistState = false` and opens expanded every time; a screen keeps the default and remembers how the user left it.
...
**The arrow is also read backwards (S2751).** The dimensions above all catch an upper layer reaching *down* past its neighbour; three more catch a lower layer reaching *up* into the screens, which no check could see until this ticket and which therefore appeared in silence - six such imports had accumulated in two watch use cases. `wear-domain-imports-ui` watches `wear/.../domain/`, `wear-data-imports-ui` watches `wear/.../data/`, and `domain-imports-ui` watches `app_v2/.../domain/`; all three live in `scripts/quality/assert-source-gates.ps1` beside the S2103 set. Each module-and-layer pair carries its own baseline so a new edge in one cannot be paid for by a cleanup in another, a level falls when an edge is unpicked, and none of them is raised without a boundary decision. The cure is the same in every case: answer with a domain value and let the upper layer map it to a route or a glyph - a table that returns domain records belongs under `domain/`, and a navigation address or a drawable never does.
...
## Wear Data Layer Bridge (S2591)

The phone and watch compile as separate modules, so their companion transport is a protocol rather than
a shared Kotlin dependency. `PhoneWearListenerService` in the phone GMS source set owns traffic arriving
on the phone; `WatchWearListenerService` owns the corresponding watch traffic. The listener declarations
in the two manifests supply the route-prefix filters that cause Google Play services to start them. A new
route outside an existing prefix needs its matching manifest filter before either listener can receive it.

Choose the transport by delivery semantics, not by payload size alone:

- **MessageClient** carries one-time commands, requests, acknowledgements, deltas, and reports. A sender
  chooses a reachable node and the receiver treats an acknowledgement or refusal outcome as the terminal
  answer to that request.
- **DataClient** carries latest state that a reconnecting peer must recover, including settings, playback
  state, pinned content, resource pages, receiver lists, and deferred outcomes. The receiver replaces or
  merges that state idempotently and removes one-shot outcome items after consuming them.
- **ChannelClient** carries file and media bytes. The file-transfer route ends with the file name, which is
  the correlation key used by the receiving side; metadata and completion acknowledgements travel on
  Messages around the channel lifecycle.

The normal structured payload is `WearEventEnvelope`: an event type, schema version, send time, and
encoded data. It lets a receiver reject an unsupported event or payload shape without treating it as a
different route. The compatible network-sources request, push, and acknowledgement channel predates this
envelope and deliberately keeps its old payload format, but its routes are still part of the common
contract.

`WearDataLayerPaths` is hand-mirrored in the phone and watch modules because they cannot share an artifact.
Every common `/fms/` route has the same name and value in both copies, even when only one listener consumes
one direction. `scripts/quality/assert-wear-wire-vocabulary-parity.ps1` compares that route map together
with the wire vocabularies; its regression suite proves a one-sided route change fails. Add a route to both
catalogues, the relevant listener, and this transport rule in the same change. Do not add a local literal
as a shortcut around the parity gate.
