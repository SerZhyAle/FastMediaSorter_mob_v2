...
- **After any build domain, never before it.** Domains carry a rank (`Build.Phone` 1 .. `Code.Scripts` 5) and `Resolve-AgentLockTopUp` refuses the descending direction outright, so in a gradle-backed generator the call goes after `Enter-BuildLockOrExit`.
...
Room schema version: 55 (`@Database(version = ..)` in `AppDatabase.kt` is the source of truth - read it rather than this line).
