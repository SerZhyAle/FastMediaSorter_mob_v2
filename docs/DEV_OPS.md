...
### The two baseline shapes and the inventory - S3438

Contract `CHECK-BASELINE` (catalog function `automated-checks`, section 3) names two shapes, and every baseline here is one of them:

- **Count ratchet** - the whole file is one number, and the check judges the count.
- **Set baseline** - one identifier per line, sorted, and the check judges membership. A large generated XML (detekt, lint) is a set only when a committed plain identifier snapshot travels beside it.

Rule 4 decides between them: a count cannot see an absorption, so a class whose baseline can be re-frozen wholesale must be a set. A count ratchet is safe here when no script can write a higher number - every writer is down-only or seed-only, or none exists - because a rise then needs a hand edit that the diff shows as the number going up. A writer that raises needs `-Reason` and prints what it accepts (rule 6), as `assert-detekt-baseline-absorption.ps1 -Update -Reason` and, since S3438, `assert-tactical-step-form.ps1 -UpdateBaseline -Reason` do.

`scripts/quality/baseline-inventory.jsonl` declares every baseline: `file`, `shape` (`count`|`set`|`xml`), `owner`, `write` (`down-only`|`seed-only`|`hand-only`|`raise-with-reason`|`guarded`|`wholesale`), `verdict` (`safe`|`exception`), and for an exception its `carrier` ticket and `reason`. `scripts/quality/assert-baseline-inventory.ps1` refuses a baseline with no row, a row with no file, and a `wholesale` writer declared `safe`; it runs per ticket from `post-change.ps1` in fixed-input form whenever the set carries a baseline or the inventory. The audit that seeded it (2026-09-23, 112 files) found two unguarded lint XML baselines (closed by S3459: identifier snapshots plus `assert-lint-baseline-absorption.ps1`) and eight set or floor baselines whose writer accepts wholesale with no reason (carrier S3460); each is a dated exception row. Discovery walks `scripts/` and `config/` on disk as well as asking git, because two baselines had sat under the `*token*` `.gitignore` blanket and never reached a commit.

A floor (`allfeatures-sync`: the ledger must not shrink) or a retention set (`allfeatures-gate`: a gated record must keep its gate) is not a third shape - it is the same two shapes judged the other way round, so the harmful re-freeze is the one that lowers or removes.
...
- **Fixed input, FATAL only when a declared input is in the set:** every gate dot-sourcing `scripts/quality/lib/fixed-input-scope.ps1` and invoked through `Invoke-FixedInputGate` - today `baseline-inventory`, `doc-pin-drift`, `flavor-matrix-doc`, `launcher-reset-coverage`, `oss-notices`, `rule-digest-sync`, `wear-canonical-key-parity`, `wear-settings-parity`, `wear-wire-vocabulary-parity`. Outside the set the gate prints its findings and returns child exit 3, which the closure reports as an advisory naming the script to re-run project-wide. Read the membership off the dot-source, never off this list: S2824 gave three gates the fork, S2827 a fourth and S2828 four more inside two days, and the name lists in `CLAUDE.md` and `AGENTS.md` were stale both times.
...
- **An entry can be declared for one flavor, and `outOfFlavor` is what the walk records elsewhere (S3358).** A `screens[]` entry may carry `flavors` - the builds it is walkable in, absent meaning all of them - plus the catalog row it depends on, `homeSection` or `wearApp`. Where the installed build is not on the list the walk records `outOfFlavor` and moves on without tapping, without moving its position and without spending its `backAfter`: the Home section or Apps program it reaches for is not drawn in that artifact, so there is no control and never was. Scored by nothing, and reported beside the coverage line for the reason coverage exists - a sweep that opened seven of twenty-eight entries has to account for the rest. Why the field had to exist: S3178 put twelve rows behind a `WearRestrictedCapabilities` answer the store build returns false for, the list went on declaring all of them, and the sweep that judges `standard` by default returned observed 7, failed 2, unreachable 18 on `emulator-5556` 2026-09-20 - eighteen verdicts about screens that were never in the build, and any real finding buried among them.
- **Whether a `flavors` claim is TRUE is a unit test, not a script (S3358).** `wear/src/test/..WearWalkContract.kt` reads the screen list back and `StoreBoundaryTest` / `NoLegalBoundaryTest` hold it to `HomeSectionCatalog` and `WearAppCatalog` **running**, with their own flavor's capabilities bound: an entry is walkable in a flavor exactly when that flavor draws the row it names. The two cases contradict each other by construction, so narrowing every entry until nothing is walked fails the sideload half. It is a test rather than a dimension of the gate above because a PowerShell re-reading of `HomeSectionCatalog` would be a second implementation of the rule it checks - and a second implementation is what the screen list already was. `wear/build.gradle.kts` declares the list as an input of the unit test tasks: it lives outside the module, so without that an edit to the screen list alone left the task UP-TO-DATE and the check never ran.
...
### Automated phone UI sweep - S2380

The phone equivalent of the walk above, and deliberately not the same code (strategic ADR-2). It photographs every declared phone screen in every combination of a declared matrix - screen profile, language, colour theme, orientation - and hands the frames to the existing screenshot review. Five stages, each its own script, joined by files on disk rather than by calls, so the corpus can be re-read without re-running the walk:
...
pwsh -NoProfile -File scripts/devtest/ui-sweep-compress.ps1 -CorpusDir temp/S2380/sweep -OutDir temp/S2380/sweep-compressed
```

- **Every dimension is declared in one file, never in the walker.** The matrix and its shortened subsets are `scripts/devtest/ui-sweep-matrix.json`; the screen geometries are `scripts/devtest/ui-sweep-profiles.json`; the screens, their expected markers and their exclusions with reasons are `scripts/devtest/ui-sweep-screens.json`. A third form factor, a tenth theme or a new screen is a record in one of those three, not an edit to a script.
- **`-Subset quick` is the shortened run** the owner asked for - one profile, one language, one theme - and yields a corpus of the same shape as the full matrix, which is what makes it usable as a check rather than only as a sample. `ru-dark` is the middle slice that exercises the second language and a dark theme on both benches.
- **Exit codes, and why 1 and 2 must not be merged.** `ui-sweep-bench.ps1`: 0 matched or created, 1 the attached device is not the declared profile, 2 could not look. `ui-sweep-walk.ps1`: 0 every combination walked, 1 at least one product defect - a screen opened and its expected token was absent, 2 could not verify - an unreachable control, an unconfirmed combination state, an unreadable tree, or the row-count identity failing. `ui-sweep-compress.ps1`: 0 compressed, 1 the corpus is present but unusable (no journal, output outside `temp/`), 2 no corpus found. "The app is broken here" and "the walk never got to judge" call for opposite reactions, on every one of the three.
- **The walk's journal is read beside the corpus, never after it.** `temp/S2380/sweep/sweep-journal.json` carries one row per screen per combination with an explicit outcome - `observed`, `failed`, `unreachable`, `manual`, `skipped`, `refused-state`, `refused-secure` - and the run asserts rows equal screens times combinations before it scores itself. A proposal about a screen that never opened and one about a screen photographed in every combination are different claims.
- **A refusal to reach the start screen writes down what it saw.** When `Wait-ForMainScreen` gives up, the walk persists the last tree it read as `temp/S2380/sweep/refusal-main-screen-NN.xml`, beside a `.txt` naming the markers it was looking for, the node count, the foreground activity and the window that held focus - and the failure reason carries that path. Read the tree, never the sentence: one and the same refusal text covers a wizard standing in front of the matrix, a marker not drawn at this width, and a launch that merely outran the wait budget, and S2380 diagnosed two of those three wrongly before the artifact existed.
...
`ocr-overlay-accuracy.md` naming the report's date and path - a bound with no dated report behind it
does not exist.
...
**Every regeneration names what it accepted (S3459).** Each XML has a committed identifier snapshot beside it, `<module>/lint-baseline.ids`: one `<issue id>|<file>|<digest>` per finding, sorted, where the digest covers the message and the flagged source line but never the line number, so an unrelated edit above a finding does not read as a new one. `scripts/quality/assert-lint-baseline-absorption.ps1 -Gate` fails when the XML carries an identifier the snapshot does not - set semantics, because a re-freeze can shrink the file while absorbing new debt. It runs in `.\a.ps1 fg` and, fatally, in `post-change.ps1` whenever the changed set carries either file. Both regeneration paths require a reason and re-seed the snapshot through `-Update -Reason`, which prints every newly accepted identifier for the dev-log row: `check-lint.ps1 -Regenerate -Reason '<why>'` locally, the `reason` input of the CI dispatch remotely.
...
- **Regenerate the forwarders** after a plugin update that adds or renames a harness script:
  ...
  writing.
- **How the rules arrive, and when the stamp is stale** (contract `RULE-DELIVERY` in the shared
  catalog, section 5, rules 1-7). The plugin version is the only delivery carrier: `claude plugin update`
  compares that number and nothing else, so a canon edit that did not go through the canon's
  ...
  without that text leaves the ticket blocked.
- **Re-stamp the adoption** after a plugin update, once the changed rule docs are reconciled (the
