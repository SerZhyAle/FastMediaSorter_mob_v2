...
4. Обновляет `docs/WHATS_NEW.md` + **обязательно** зеркала `WHATS_NEW-ru.md` и `WHATS_NEW-uk.md`. Из них генерятся локализованные fastlane-changelog'и; если зеркала не обновить - в Play/IzzyOnDroid уедут заметки прошлого релиза.
5. Обновляет `README.md` (+ `README-ru.md`/`README-uk.md`, если там есть блок версии).
