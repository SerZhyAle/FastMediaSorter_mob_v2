...
- [ ] **Small Controls Mode in BrowseActivity**
  - When settings.smallControls=true, all buttons in Browse top/bottom command panels should reduce in size
  - Affects: BrowseActivity toolbar buttons and bottom panel buttons

- [ ] **SMB Connection Blocking After Errors**
  - Issue: After certain SMB errors, connection becomes blocked until app restart
  - Need to investigate and implement proper recovery mechanism

## 🚀 Recent Fixes

### Build 2.25.1119.xxxx ✅
- ✅ **FEATURE: IP Address Input Filter**
- **Implementation**: Created IpAddressInputFilter for server/host input fields
- **Behavior**: 
  - Allows: digits, dots, letters (a-z, A-Z), dash, underscore
  - Auto-replaces: comma → dot, space → dot
  - Blocks: all other special characters
- **Applied to**:
  - AddResourceActivity: etSmbServer, etSftpHost (replaced old InputFilter+TextWatcher)
  - EditResourceActivity: etSmbServerEdit, etSftpHostEdit
- **User Experience**: Typing "192,168,1,1" or "192 168 1 1" automatically converts to "192.168.1.1"
- **Code Improvements**: Simplified from ~90 lines (inline InputFilter + 2 TextWatchers) to ~3 lines per field
