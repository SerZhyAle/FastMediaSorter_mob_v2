...
{% include lang-switcher.html doc="README" dir="/docs/" current="uk" %}
...
Встановлюєте APK напряму? Android попереджає про пакет, який бачить уперше, - [чому з'являється попередження і що натискати](INSTALL_TRUST-uk.md).

**📘 Документація користувача (англійською):** [покрокові інструкції до всіх функцій, з пошуком](https://serzhyale.github.io/FastMediaSorter_mob_v2/documentation/)

## Про проект

**FastMediaSorter v2** - повноцінна оболонка Android-пристрою. Вона займає домашній екран, відтворює вашу медіатеку, відкриває трансляції, запускає ваші програми, спілкується з годинником, стежить за станом пристрою й керує всіма вашими файлами - у локальних теках, на мережевих дисках (SMB, SFTP, FTP) і в хмарних сховищах (Google Drive, OneDrive, Dropbox).

В її основі вісім стовпів: оболонка пристрою, медіаплеєр, трансляції, запуск програм, заміна штатних програм, супутник на годиннику, моніторинг пристрою та повний файловий менеджер. Сортування файлів в усіх цих джерелах - те, з чого застосунок починався, і воно й досі лишається фундаментом для всього іншого, але вже не вичерпує його.

## Що замінює FastMediaSorter 🧩

FastMediaSorter - повноцінна оболонка Android-пристрою: домашній екран, медіаплеєр, трансляції, запуск програм, супутник на годиннику, моніторинг пристрою та повний файловий менеджер в одному застосунку. Цей один застосунок покриває те, для чого зазвичай потрібен десяток окремих програм. Відомі застосунки нижче наведені лише як впізнавані орієнтири для кожної можливості, а не як порівняння.
...
- **Інтернет-радіо / IPTV-плеєр** (TuneIn, Shoutcast, Online Radio, RadioDroid, VLC network streams, IPTV-плеєри) - виділений екран Трансляцій для відтворення інтернет-аудіо та відео: http-радіо, HLS/DASH, RTSP; вбудований мінібанер з ICY-метаданими (назва станції/треку); каталог трансляцій із пошуком та фільтрами.
...
- **Набір віджетів робочого столу** - віджети швидкого запуску та стану для диктофона, OCR з камери, операцій за розкладом, поточного треку та фоторамки.
...
## Що нового в v2.60.9240.148 (вересень 2026)

**Нове:**
Зашифровані контейнери (.fd-sec), копіювання та перенесення файлів на годинник, камера телефона з годинника, екран запланованих операцій з історією запусків, живий годинник на затемненому екрані, меню «Пуск» та оформлення значків у лаунчері, керування з D-pad і клавіатури, значок регіональних обмежень у потоків, зупинка зворотного зв'язку під час трансляції, закріплення ключа SFTP-сервера, переклад поверх зображення не виходить за край.

**Виправлено:**
Звук сирени SOS, оновлення списку після фонових операцій, повідомлення про доступ до пам'яті показується один раз, розпізнавання типу потоку, жести водяного ліхтарика на годиннику, перемикання рушіїв OCR.

## Попередній випуск: v2.60.9191.534 (вересень 2026)

**Нове:**
Передавання файлів між пристроями через Google Drive, сигнал SOS з телефона, повніша резервна копія налаштувань з експортом та імпортом у файл або Drive, ярлики застосунку, що ведуть прямо в підпрограми, повніший стартовий робочий стіл з готовими профілями та рядками стартової панелі, швидше редагування робочого столу з перетягуванням за край і перестановкою цілих секцій, вибір вікна лаунчера та окремий тайм-аут екрана під час заряджання, один екран мовлення з переглядом із камери та запам'ятованим вибором об'єктива, окремий розділ налаштувань мовлення, що переживає відновлення з резервної копії, простіша передача ефіру через файл-дескриптор і посилання імпорту, і притлумлений плеєр, на якому все одно видно годинник.

**Виправлено:**
Нульові мініатюри, список ліцензій, що відкривається без затримки, усунення потоку голосових оголошень на екранах плеєра і мовлення, збереження розміру значків сітки та інтервалу фонової синхронізації, SOS і висота панелі задач, що переживають відновлення з копії, повне покриття функції «Запобігти сну», коректне відновлення анімацій після заряджання, кнопки діалогів, що йдуть за мовою застосунку, і точна назва і бітрейт мовлення.

[Повні примітки до випуску →](WHATS_NEW-uk.md)

## Версія для Windows 🖥️

Шукаєте рішення для настільного комп'ютера? Спробуйте **Fast Media Sorter for Windows** (раніше FastMediaSorter LITE) - легкий додаток Windows Forms для швидкого сортування, перегляду та керування зображеннями та відео:

🔗 **[Fast Media Sorter for Windows](https://github.com/SerZhyAle/FastMediaSorter_Lite)**

📄 [Як опублікувати папки ПК для Android](https://serzhyale.github.io/FastMediaSorter_Lite/publish-folders-android.html) - поділіться папками ПК із застосунком через SFTP (companion-імпорт / скан QR на телефоні).
...
- [Редакції застосунку](#редакції-застосунку-)
...
FastMediaSorter v2 виходить у **семи редакціях**: п'ять для повсякденних телефонів і планшетів (Standard, Lite, Photos, Legacy, FOSS) плюс дві збірки для шоломів і sideload - VR і noLegal. Канонічна матриця можливостей генерується з файлу збірки: [FLAVOR_MATRIX.md](FLAVOR_MATRIX.md):

| Редакція | Опис | Примітки |
|--------|------|----------|
| **Standard** | Повнофункціональний реліз | Найширший набір можливостей для медіа, документів, OCR, перекладу та хмарних інтеграцій |
| **Lite** | Полегшений реліз | Тільки локальні файли - відео, аудіо та зображення; без мережевих джерел, хмари, документів і Трансляцій |
| **Photos** | Фото-орієнтований реліз | Тільки зображення, з SMB/FTP/SFTP і хмарою; без відео та без аудіо |
| **Legacy** | Реліз для сумісності | Той самий набір можливостей, що у Standard, включно з SMB/FTP/SFTP і хмарою (Google Drive, Dropbox, OneDrive); зібраний для Android 6/7 (API 23+) |
| **FOSS** | Реліз для каталогу F-Droid | Без пропрієтарних SDK: локальне медіа, документи, EPUB і SMB/FTP/SFTP; без хмари, без Трансляцій, без OCR, без перекладу, без Cast і без компаньйона Wear OS |
| **VR** | Store-збірка для шолома | Повний набір медіа для шоломів; без Google Cast і без компаньйона Wear OS |
| **noLegal** | Sideload-збірка | Усе, що є в Standard, плюс іммерсивний плеєр OpenXR і можливості лише для sideload |

### Яку редакцію вибрати?
...
- **FOSS**: Обирайте в каталозі F-Droid, якщо потрібна збірка без пропрієтарних SDK
- **VR**: Обирайте для XR-шолома - store-збірка без Cast і без підтримки Wear
- **noLegal**: Тільки sideload - беріть, якщо потрібен іммерсивний плеєр OpenXR

Для точної матриці можливостей за редакціями використовуйте канонічні документи:

- [Каталог можливостей (канонічний)](FEATURES-uk.md)
- [How-To (таблиця доступності за редакціями)](HOW_TO-uk.md)
- [Quick Start (вибір редакції)](QUICK_START-uk.md)
- [Обмеження програми](LIMITATIONS-uk.md)

> 🧭 **Перший запуск:** прямо під вибором мови додаток запропонує вибрати **профіль пристрою** (телефон, планшет, ТВ, авто, фоторамка, VR та інші), який підбере для вас стартові налаштування - змінити можна будь-коли в Налаштуваннях. Див. [Перший запуск: виберіть профіль пристрою](QUICK_START-uk.md#перший-запуск-виберіть-профіль-пристрою-30-секунд-).
...
| Редакція | Ім'я файлу | Опис |
|--------|------------|------|
| **Standard** | `FastMediaSorter_standard_release.zip` | Повний функціонал (Хмара, OCR, EPUB) |
| **Lite** | `FastMediaSorter_lite_release.zip` | Тільки локальні медіа (без мережі, хмари, документів і Трансляцій) |
| **Photos** | `FastMediaSorter_photos_release.zip` | Тільки зображення, з мережею (SMB/FTP/SFTP) і хмарою |
| **Legacy** | `FastMediaSorter_legacy_release.zip` | Ті самі можливості, що у Standard, включно з мережею (SMB/FTP/SFTP) і хмарою; Android 6/7 (API 23+) |
...
- ⭐ **Система обраного:** Позначайте важливі файли як обрані та швидко отримуйте доступ до них зі спеціальної вкладки, яка агрегує вибране з усіх джерел.
- 🔒 **Захист ПІН-кодом:** Захистіть окремі ресурси ПІН-кодами доступу, щоб запобігти несанкціонованому перегляду та редагуванню.
...
- 🗣️ **Дії AppFunctions для асистента (Android 16+):** Застосунок оголошує дії, які може викликати системний асистент, - знайти медіафайл, відкрити файл або відкрити папку на комп'ютері, - щоб асистент вашого пристрою знаходив і відкривав ваш контент просто за голосовою командою.
...
- 🌐 **Авто-переклад:** Миттєвий переклад тексту із зображень, PDF і текстових файлів повністю на пристрої: **Tesseract** читає текст латиницею й кирилицею, а Google ML Kit його перекладає.
...
- 🏠 **Режим лаунчера (домашнього екрана):** Дозвольте застосунку стати лаунчером - домашнім екраном пристрою (збірки Standard і noLegal): власний робочий стіл з ярликами ресурсів, що відкриваються прямо в перегляд, слайд-шоу чи відтворення, гаджетами, що змінюють розмір, на кшталт годинника й погоди, комірками контактів без потреби в дозволі на контакти, сіткою застосунків і панеллю задач. Вимкніть будь-коли - і Android відновить попередній домашній екран.
...
- 📻 **Трансляції (Інтернет-радіо та відео):** Виділений екран для http(s)-радіо, HLS/DASH VOD та RTSP-джерел. Додавайте URL вручну або імпортуйте .m3u-плейлист / кураторський каталог. Вбудоване програвання аудіо безпосередньо зі списку з липким міні-баннером ICY now-playing. Фільтрація за категорією та мовою. Підтримується в Standard, Legacy, VR і noLegal; відсутнє у Lite і Photos.
- 📲 **Захоплення екрана та запис:** Панель жестів біля лівого краю - скриншоти, швидкі фото, обрізання з надсиланням і запис екрана/голосу/відео, не покидаючи поточний файл.
- 📊 **Статистика використання (за бажанням):** Локальна панель: відсортовані файли, звільнене місце, час відтворення - дані не покидають пристрій, поки ви самі їх не експортуєте.
- 🧹 **Пошук дублікатів і очищення за розміром:** Сканування за вмістом (розмір, швидкий хеш, SHA-256) з ручним або автоматичним видаленням, плюс видалення файлів більших/менших за заданий розмір.
...
- **Трансляції:** HTTP(S) прогресивне аудіо (MP3/AAC), Icecast/Shoutcast ICY, HLS (.m3u8), DASH, RTSP; .m3u плейлисти (Standard / Legacy / VR / noLegal); недоступно у Lite і Photos.
- **Документи:** TXT, MD, LOG, JSON, XML, PDF, **EPUB**
- **Бінарні файли** (Режим «Усі файли»): ZIP, RAR, 7z, TAR, GZ, ISO, DMG, IMG, APK, EXE, DLL, SO та 60+ інших форматів
...
- [Завантаження збірок (EN)](DOWNLOADS.md)
...
- [Веб-портал Лаунчера](launcher/index-uk.md)
- [Веб-портал Wear OS](wear/index-uk.md)
...
Компаньйон Wear OS давно переріс простий перегляд локальних файлів - тепер це повноцінний другий екран для вашої бібліотеки.

- Перегляд і відтворення папок та обраного зі спареного телефону, власного сховища годинника та мережевих ресурсів SMB/FTP/SFTP, до яких годинник підключається сам по Wi-Fi
- Хмарні ресурси залишаються на телефоні - власного хмарного клієнта в годинника немає; хмарний файл потрапляє на годинник, лише якщо надіслати його з телефону через «Надіслати до..»
- Пересилання файлів між телефоном і годинником, живе мовлення прямо з годинника та дрібні вбудовані інструменти (калькулятор, монітор мережі, міні-гра) без відкриття застосунку на телефоні
- Інтерфейс та поведінка оптимізовані для компактних круглих екранів
- Окремий веб-портал, покрокові інструкції та документація

Медіа, мережеві ресурси та пересилання файлів є в повній версії застосунку для годинника (APK напряму). Версія з Google Play - невеликий перший випуск: калькулятор, секундомір, міні-гра та налаштування; що є в якій версії, позначено на [веб-порталі Wear OS](wear/index-uk.md).
...
- JDK 17+
- Android SDK 35
- Мінімальна версія Android: 8.0 (API 26) для Standard/Lite/Photos/VR/noLegal; 6.0 (API 23) для Legacy
...
- **OCR та Переклад**: Tesseract4Android (витягування тексту латиницею та кирилицею), Google ML Kit (переклад і визначення мови)
