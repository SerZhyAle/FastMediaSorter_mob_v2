...
**Область:** Все видимые пользователю строки в каждом флейворе (текущий набор перечисляет [FLAVOR_MATRIX.md](FLAVOR_MATRIX.md)), EN/RU/UK. Исключения: юридические тексты, Условия использования, машинно-читаемые технические артефакты (манифесты, метаданные).
...
- Обычный дефис `-` во всех локалях; никогда длинные тире `–` (en), `-` (em) или `―` (горизонтальная черта).
...
- `docs/termbase.jsonl` - первичный источник канонических терминов для корпуса пользовательской документации: одна запись на понятие с каноническим названием, простым определением, запрещёнными синонимами, ключами строк интерфейса, где оно показано, и утверждёнными названиями RU/UK. Документация пишется на американском английском (en-US).
- Каноническое название - то, которое пользователь видит на экране. Если сам экран непоследователен - два названия одной вещи или британское написание, - база терминов фиксирует один канонический выбор, а отклоняющаяся строка считается дефектом и исправляется в самой строке.
- Правила ниже - сторона того же глоссария для строк интерфейса, и для строк они остаются в силе.
...
### 7.2 Пополнение базы терминов (S2974)

Автор страницы документации, встретивший понятие:

1. Ищет его в `docs/termbase.jsonl` по `id`, `canonical_en` или `forbidden_synonyms_en`.
2. Если понятия нет, добавляет одну полную запись - все поля схемы, ключи строк интерфейса, где оно показано, и названия RU/UK из `values-ru/` и `values-uk/`.
3. Запускает `pwsh -NoProfile -File scripts/quality/assert-docs-termbase.ps1`; код выхода 0 обязателен до закрытия страницы.
4. Использует на странице каноническое название.

- Намеренная цитата запрещённого слова, например объяснение, как экран называют другие приложения, исключается аннотацией `<!-- termbase-ignore: <слово или id термина> -->` на той же строке или строкой выше.
- В запрещённые синонимы попадают только слова, неверные везде в корпусе. Слово, верное в другом смысле - «widget» для виджета главного экрана Android, - получает собственную запись. Британское написание записывается как `word => replacement`, чтобы гейт предлагал слово en-US, а не название термина-носителя.
