...
> **Это не единственный приёмник `text/plain` (S1175).** В листе шаринга есть вторая, отдельно подписанная запись - `activity-alias` `.LauncherPlaceShare`, ведущая на `LauncherPlaceShareActivity`. Она принимает место, отправленное из приложения карт, и кладёт его ярлыком на домашний экран лаунчера; описанный здесь конвейер загрузки по ссылке при этом не запускается. Запись включается и выключается вместе с HOME-компонентом, поэтому вне режима лаунчера её в листе просто нет. Разведены они намеренно: угадывать по тексту, что имел в виду пользователь - место или ссылку на скачивание - невозможно.
