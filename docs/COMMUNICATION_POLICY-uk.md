...
- Приклади:
  - ✓ «Тут поки нічого немає. Додайте ресурс - папку пристрою або мережеве джерело, - щоб почати.»
  - ✗ «Нічого не знайдено.»
...
- `docs/termbase.jsonl` - першоджерело канонічних термінів для корпусу користувацької документації: один запис на поняття з канонічною назвою, простим визначенням, забороненими синонімами, ключами рядків інтерфейсу, де воно показане, і затвердженими назвами RU/UK. Документація пишеться американською англійською (en-US).
- Канонічна назва - та, яку користувач бачить на екрані. Якщо сам екран непослідовний - дві назви однієї речі або британське написання, - база термінів фіксує один канонічний вибір, а рядок, що відхиляється, вважається дефектом і виправляється в самому рядку.
- Правила нижче - бік того самого глосарію для рядків інтерфейсу, і для рядків вони залишаються чинними.
...
### 7.1 Ресурс і папка (S0799)

- **Ресурс** - внутрішня сутність FastMediaSorter: зареєстрована адреса (локальна, мережева або хмарна), яку користувач додав вручну або імпортував. Ресурс може бути джерелом перегляду, отримувачем «Швидкого сортування», медіатекою або віртуальним агрегатом (Уся музика, Усі відео, обране, трансляції). Ресурс указує на одну папку або гілку папок. Використовуйте «ресурс», коли йдеться про зареєстрований запис застосунку: додати / змінити / копіювати / видалити / вибрати / оновити ресурс, його назву / тип / шлях / PIN, список ресурсів.
- **Папка** (директорія) - директорія файлової системи, що існує незалежно від застосунку: локальна папка Android або папка на віддаленому сервері SMB / (S)FTP / хмари. Використовуйте «папку» лише для справжньої роботи з директоріями: системний або ручний вибір папки, папка, на яку вказує ресурс, сканування підпапок, створення папки на диску, ідентифікатори папок хмарних провайдерів, перехід до поточної / батьківської директорії.
- Ніколи не називайте ресурс «папкою»: наприклад, дія «Додати ресурс» не має відкривати діалог із заголовком «Додати папку».
- Розділення підкріплене двома значками: `ic_resource` (стос-колекція) позначає поняття ресурсу; `ic_folder` - справжню папку. Не плутайте їх (S0842).

### 7.2 Поповнення бази термінів (S2974)

Автор сторінки документації, що натрапив на поняття:

1. Шукає його в `docs/termbase.jsonl` за `id`, `canonical_en` або `forbidden_synonyms_en`.
2. Якщо поняття немає, додає один повний запис - усі поля схеми, ключі рядків інтерфейсу, де воно показане, і назви RU/UK з `values-ru/` та `values-uk/`.
3. Запускає `pwsh -NoProfile -File scripts/quality/assert-docs-termbase.ps1`; код виходу 0 обов'язковий до закриття сторінки.
4. Використовує на сторінці канонічну назву.

- Навмисну цитату забороненого слова, наприклад пояснення, як екран називають інші застосунки, виключають анотацією `<!-- termbase-ignore: <слово або id терміна> -->` на тому самому рядку або рядком вище.
- До заборонених синонімів потрапляють лише слова, хибні скрізь у корпусі. Слово, правильне в іншому сенсі - «widget» для віджета головного екрана Android, - отримує власний запис. Британське написання записують як `word => replacement`, щоб гейт пропонував слово en-US, а не назву терміна-носія.
