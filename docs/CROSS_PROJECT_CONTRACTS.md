...
| `LIVE-BROADCAST` | 0.13 (draft) | `live-broadcast/` | producer | Never publish a loopback address; no in-stream metadata; one descriptor across link, file and barcode. A stream kind moves from `[PROPOSED]` to `[CONTRACT]` only through the handshake in section 6, with the consumer verifying against a live broadcast. |
...
| `MEDIA-CLASSIFICATION` | 0.9 (draft) | `media-classification/` | owner; producer and consumer | Resolve files and URLs into canonical media categories with case-insensitive lowercase extension matching; exclude system junk (.DS_Store, Thumbs.db, temp files); recognize sidecars without reclassifying parent media. |
| `DIAGNOSTIC-REPORT` | 0.9 (draft) | `diagnostic-report/` | owner / producer | Bounded diagnostic archives (max 1 MB); redact credentials, private keys and personal filesystem paths; export only on explicit user request. |
