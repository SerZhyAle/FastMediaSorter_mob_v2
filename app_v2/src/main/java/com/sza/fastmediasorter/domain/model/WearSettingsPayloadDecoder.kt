 ...
 *
 * Both are fixed in one pass: the offending keys are removed BEFORE deserialization and reported
 * separately, so what reaches Gson is always a well-formed subset and what reaches the caller always
 * says which keys were real.
 */
class WearSettingsPayloadDecoder(private val gson: Gson = Gson()) {
    ...
    companion object {
         ...
         * reported as [WearSettingsFieldIssue.UNKNOWN_KEY] and dropped, and the watch copy of this file
        ...
        private val EXPECTED: Map<String, JsonKind> = linkedMapOf(
            ...
            "dimClockOverlayEnabled" to JsonKind.BOOLEAN,
            "dimClockSecondsVisible" to JsonKind.BOOLEAN
        )
