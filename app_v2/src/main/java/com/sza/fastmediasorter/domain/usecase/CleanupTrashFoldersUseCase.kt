...
class CleanupTrashFoldersUseCase @Inject constructor() {
     ...
     * @param maxAgeMs Maximum age of trash folders to keep (default 5 minutes)
                    ...
                    if (file.isDirectory) {
                        if (file.name.startsWith(TRASH_PREFIX)) {
                            ...
                            if (timestamp != null) {
                                ...
                                if (age > maxAgeMs) {
                                    // Trash folder is old enough, delete it
                                    if (deleteRecursively(file)) {
                                        deletedCount++
                                        Timber.d("Deleted old trash folder: ${file.name} (age: ${age}ms)")
                                    } else {
                                        Timber.w("Failed to delete trash folder: ${file.name}")
                                    }
                                } else {
                                    Timber.d("Keeping recent trash folder: ${file.name} (age: ${age}ms)")
                                }
                            } else {
