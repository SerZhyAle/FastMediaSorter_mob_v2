...
class RecordingIndicatorOverlayManager(private val activity: FragmentActivity) {
    ...
    private val pauseResumeButton: MaterialButton by lazy { activity.findViewById(R.id.recordingIndicatorPauseResume) }
    ...
    private fun applySafeBoundsInsetsOnce() {
        if (insetsApplied) return
        insetsApplied = true
        val baseTopMargin = (root.layoutParams as? ViewGroup.MarginLayoutParams)?.topMargin ?: 0
