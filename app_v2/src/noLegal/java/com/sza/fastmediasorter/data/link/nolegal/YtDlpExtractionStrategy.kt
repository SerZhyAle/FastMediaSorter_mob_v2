...
class YtDlpExtractionStrategy @Inject constructor(
            ...
            val result = EXECUTOR.submit<Any> {
                runCatching {
                    ...
                    if (msg.contains("There is no video in this post", ignoreCase = true) ||
                        ...
                        Timber.d("S0187: yt-dlp format-unavailable cascade to NewPipe url=%s", url.take(80))
                        Timber.d(
                ...
                is DelegateParams -> {
                    ...
                    when {
                        ...
                        delegated is OpenResult.Blocked &&
                            ...
                            downloadViaPython(url, cookieFile, result.safeTitle, result.ext, sessionUa)
                        }
                        delegated is OpenResult.Blocked &&
     ...
     * No progress callbacks — progress is reported as 0/unknown during this phase.
     */
