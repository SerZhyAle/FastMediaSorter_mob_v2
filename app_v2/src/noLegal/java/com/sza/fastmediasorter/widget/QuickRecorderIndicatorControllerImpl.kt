 ...
 * pause/resume and cancel stay hidden, this indicator only ever stops+saves. noLegal declares
 * `SYSTEM_ALERT_WINDOW` unconditionally, so this passive view needs no `TYPE_ACCESSIBILITY_OVERLAY`
 * fallback - that path exists on [ScreenGestureOverlayControllerImpl] for its touch-intercepting
 * gesture strip, not for a plain view like this one.
 */
