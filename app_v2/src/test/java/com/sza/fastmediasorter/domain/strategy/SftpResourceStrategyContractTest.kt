...
/**
 * A2-T5/T7/T8/T9: Field schema and validation contract tests for SFTP strategy.
 */
class SftpResourceStrategyContractTest : ResourceEditorContractTestBase() {

    private lateinit var strategy: SftpResourceStrategy

    @Before
    fun setUp() {
        strategy = SftpResourceStrategy()
    }
    ...
    @Test
    fun `fieldSchema - CLOUD_PROVIDER is absent for SFTP type`() {
        assertFieldAbsent(strategy.fieldSchema(), ResourceFieldKey.CLOUD_PROVIDER)
    }

    @Test
    fun `fieldSchema - CLOUD_FOLDER is absent for SFTP type`() {
    ...
    fun `validate - blank name produces NAME error`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, name = ""))
    ...
    fun `validate - blank host produces HOST error`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, host = ""))
        assertFalse(result.isValid)
    ...
    fun `validate - blank path produces PATH error`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, path = ""))
        assertFalse(result.isValid)
    ...
    fun `validate - port null accepted`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, port = null))
        assertTrue("port=null should be accepted", result.isValid)
    }

    @Test
    fun `validate - port 22 is valid`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, port = 22))
        assertTrue("Standard SFTP port 22 should be valid", result.isValid)
    }

    @Test
    fun `validate - port 0 produces PORT error`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, port = 0))
        assertFalse(result.isValid)
    ...
    fun `validate - port 65536 produces PORT error`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP, port = 65536))
        assertFalse(result.isValid)
        assertTrue(result.fieldErrors.containsKey(ResourceFieldKey.PORT))
    }

    @Test
    fun `validate - valid SFTP form produces no errors`() {
        val result = strategy.validate(networkForm(ResourceType.SFTP))
