...
class NativeDiagnosticXrRuntime @Inject constructor() : DiagnosticXrRuntime {
    ...
    override fun setHudQuadSize(widthMeters: Float, heightMeters: Float, verticalOffsetMeters: Float) {
        if (!isNativeAvailable) return
        runCatching { nativeSetHudQuadSize(widthMeters, heightMeters, verticalOffsetMeters) }.onFailure {
    ...
    private external fun nativeSetHudQuadSize(
        widthMeters: Float,
        heightMeters: Float,
