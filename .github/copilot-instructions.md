...
| Locating a Kotlin class or feature before grepping, planning a refactor/decomposition, auditing who injects a type, or refreshing class-catalog metadata after code changes | `/catalog` | **Mandatory** - query `dev/CATALOG/scripts/query.ps1` before any global Search/grep for Kotlin classes; run `scripts/catalog_sync.ps1 -Module <app_v2\|wear>` after every `.kt` change |
| Quickly fixing an existing bug, regression, crash, or already-defined UI/code issue when a narrow local patch is enough and bureaucracy should be skipped | `/skill-fix` | **Optional** - preferred fast-patch path: diagnose locally, patch the smallest slice, skip dev log/spec/docs/build/git, and use only focused local validation; refuses new features, broad refactors, release/build work, and unclear UI decisions |
...
<ai_core_directives>
  ...
  <strict_constraints>
    ...
    <constraint>DEV_CHANGELOG: MANDATORY. After EVERY code/config modification, run `.\scripts\add_to_dev_log.ps1 "path" "target" "description"` to log the change to `dev/CHANGELOG.md`. Execute AFTER each modification step, BEFORE moving to next task. Applies to ALL agents except `/skill-fix`, which explicitly skips log/spec/doc/build/git bureaucracy.</constraint>
    ...
    <constraint>COMMENTS_WRITE_ON_MODIFY: MANDATORY. Code comments are ENGLISH-ONLY and explain WHY, not WHAT. Add a comment ONLY for non-obvious business logic, a deliberately handled edge-case, a workaround, or an invariant the code cannot express itself. DO NOT add comments that merely restate what the adjacent line plainly does - if the code is self-explanatory, leave it uncommented. Remove or update stale comments that no longer reflect reality. Applies to ALL agents.</constraint>
    ...
    <constraint>UI_TRIGGER_ROW: MANDATORY. All toggle/switch and checkbox rows MUST follow the canonical patterns defined in `docs/ARCHITECTURE.md` § "UI Patterns - Trigger Row". SWITCH rows: prefer the reusable `SettingsToggleRow` compound view (`com.sza.fastmediasorter.ui.common.widget.SettingsToggleRow`); raw XML triplets `SwitchMaterial + TextView + ImageButton` are technical debt and must be migrated when adjacent code is touched. Layout: title=`toggler_title_text_size` (14sp), subtitle=`toggler_desc_text_size` (12sp) below the title, help-icon inline next to the title (drawable `ic_help_outline_24`; NOT rightmost). Optional trailing action slot reserved for exceptional rows. CHECKBOX rows: help-text=`text_size_small` (14sp), indent=`checkbox_subtitle_margin_start`. Help text is ALWAYS 2sp smaller than the trigger label. NEVER hardcode sp values. Applies to ALL agents.</constraint>
    ...
    <constraint>SYSTEM_BAR_SAFETY: MANDATORY. Every new or changed screen, Activity-backed dialog, fullscreen overlay, toolbar, bottom action bar, and empty/loading/error state MUST keep important text and touch targets inside Android `systemBars` plus display-cutout safe bounds in portrait and landscape. `android:fitsSystemWindows="true"` alone is NOT sufficient for targetSdk 35 / Android 15 edge-to-edge enforcement. View-based forms should use `View.applySystemBarInsetPadding()` or an equivalent `WindowInsetsCompat` listener; Compose surfaces must use equivalent `WindowInsets` padding. Immersive/player/game exceptions must explicitly own their insets and document why system bars may overlay non-critical background only.</constraint>
    ...
    <constraint>PWSH_EFFICIENCY: MANDATORY. Every `pwsh` invocation is a fresh process; cold start on Windows is 200..500 ms. Rules: (a) ALWAYS pass `-NoProfile` (e.g. `pwsh -NoProfile -File scripts/foo.ps1`) - no project script needs the user's `$PROFILE`; (b) BATCH related script calls into a single PowerShell process. In the Codex shell wrapper, prefer `& { ./a.ps1; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; ./b.ps1 }`. Only when a fresh process is truly required, use `pwsh -NoProfile -Command '& { ./a.ps1; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; ./b.ps1 }'`; (c) WRITE `$` variables literally in runnable commands - never `\$LASTEXITCODE` - and avoid double-quoted `-Command "..."` wrappers around code containing `$...` variables because outer layers may strip or interpolate them before PowerShell receives them; (d) PREFER project wrappers over chained primitive calls - the catalogue ritual uses `scripts/catalog_sync.ps1 -Module <app_v2|wear>` instead of separate `scan.ps1` + `render.ps1`. If a frequently-chained ritual lacks a wrapper, create one in `scripts/` (single-purpose, fail-fast on `$LASTEXITCODE`) and document it in `CLAUDE.md` → PowerShell Efficiency. Do NOT invent background-daemon or long-running-shell workarounds. Applies to ALL agents, ALL prompts, ALL ad-hoc shell calls.</constraint>
    <constraint>LAZY_OPTIMIZATION: MANDATORY. Optional features (AppSettings), network/cloud clients, or media-rendering UI elements must not degrade startup or waste RAM. Wrap heavy Hilt dependencies in `dagger.Lazy&lt;T&gt;`, load optional/format-specific layouts using `&lt;ViewStub&gt;` instead of eager `&lt;include&gt;` tags, and release player/media resources (`MediaPlayer`, `ExoPlayer`, Glide) immediately when paused or inactive.</constraint>
  </strict_constraints>
  ...
  <scripts_reference>
    ...
    <utility_scripts>
      ...
      .\scripts\utils\set-android-string.ps1 -Module app_v2 -Locale ru -Key "cloud_check_failed" -Value "..." [-ExpectedOldValue "..."] [-CreateIfMissing] # safe single-key strings.xml updater
      .\scripts\add_to_dev_log.ps1 "path" "target" "desc" # append row to dev/CHANGELOG.md (MANDATORY after every code change)
      .\scripts\catalog_sync.ps1 -Module <app_v2|wear>    # one-shot Catalogue sync (scan + render in one pwsh process) - prefer this over separate scan.ps1 + render.ps1
    </utility_scripts>
