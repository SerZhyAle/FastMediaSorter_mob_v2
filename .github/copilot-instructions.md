...
28. Deprecated PackageManager flags (CLAUDE.md Rule 21): no raw-int `getPackageInfo`/`getApplicationInfo`/`queryIntentActivities`/`resolveActivity` in `src/main` - use the `*Compat` helpers in `util/PackageManagerCompat.kt`. Gate: `scripts/quality/assert-deprecated-pm-flags.ps1`.
