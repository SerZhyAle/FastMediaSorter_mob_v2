...
29. Concurrent-agent locks (CLAUDE.md Rule 23): a script invoking `gradlew` takes `temp/BUILD.LOCK`; a multi-file source edit takes `temp/CODE.LOCK` via `scripts/utils/enter-code-lock.ps1 -Reason "..."`. A busy lock queues rather than refuses - `enter-code-lock.ps1` exits **4** for "queued, not your turn yet". Wait with `scripts/utils/wait-for-lock-turn.ps1` in the background and meanwhile do work needing no lock. Take the code lock immediately before an edit and release it right after. Abandon a queued intent and you must withdraw your own place - `scripts/utils/withdraw-lock-ticket.ps1 -Name <Build|Code>` (`.\a.ps1 uqb` / `uqc`): the eviction sweep judges the owning session, so a live session's dropped ticket head-blocks everyone behind it indefinitely (S2098).
