...
<ai_core_directives>
  ...
  <strict_constraints>
    ...
    <constraint>PWSH_EFFICIENCY: MANDATORY. Every `pwsh` invocation is a fresh process; cold start on Windows is 200..500 ms. Rules: (a) ALWAYS pass `-NoProfile` (e.g. `pwsh -NoProfile -File scripts/foo.ps1`) - no project script needs the user's `$PROFILE`; (b) BATCH related script calls into a single tool invocation via `pwsh -NoProfile -Command "& { ./a.ps1; if (\$LASTEXITCODE -ne 0) { exit \$LASTEXITCODE }; ./b.ps1 }"`; (c) PREFER project wrappers over chained primitive calls - the catalogue ritual uses `scripts/catalog_sync.ps1 -Module <app_v2|wear>` instead of separate `scan.ps1` + `render.ps1`. If a frequently-chained ritual lacks a wrapper, create one in `scripts/` (single-purpose, fail-fast on `$LASTEXITCODE`) and document it in `CLAUDE.md` → PowerShell Efficiency. Do NOT invent background-daemon or long-running-shell workarounds. Applies to ALL agents, ALL prompts, ALL ad-hoc shell calls.</constraint>
  </strict_constraints>
