...
<#
.SYNOPSIS
    Forwarder to the canon-shipped harness script locks\wait-for-lock-turn.ps1 (S2402).
...
Exit codes: whatever locks\wait-for-lock-turn.ps1 returns, plus 2 when the harness cannot be located.
#>
...
foreach ($szaFwdDir in $szaFwdCandidates) {
    $szaFwdProbe = Join-Path $szaFwdDir 'locks\wait-for-lock-turn.ps1'
...
if (-not $szaFwdTarget) {
    ...
    if ($szaFwdCheckout) {
        ...
        $szaFwdProbe = Join-Path $szaFwdCandidates[-1] 'locks\wait-for-lock-turn.ps1'
        if (Test-Path -LiteralPath $szaFwdProbe) { $szaFwdTarget = $szaFwdProbe }
    }
}
if (-not $szaFwdTarget) {
    Write-Host "wait-for-lock-turn.ps1: the SZA harness is not installed - looked in:" -ForegroundColor Red
