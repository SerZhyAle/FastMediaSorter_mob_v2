...
- Line shape: `rel  ticket  changed  status`. `rel` = release package number - the `current-next-release:` marker names the open one and the `DEBUG-v0NN` branch only labels it, so the two may differ by one or more after a package ships without a branch (a Wear release does exactly that). `--` = not scheduled. `ticket` = the spec file name without its extension. `changed` = the date the STATUS last moved, not the date the spec text was edited.
