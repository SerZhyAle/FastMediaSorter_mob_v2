...
<#
.SYNOPSIS
    Forwarder to the canon-shipped harness script spec_catalog/check-probe-absent.ps1 (S2402).
...
Exit codes: whatever spec_catalog/check-probe-absent.ps1 returns, plus 2 when the harness cannot be located.
#>
...
foreach ($szaFwdDir in $szaFwdCandidates) {
    $szaFwdProbe = Join-Path $szaFwdDir 'spec_catalog/check-probe-absent.ps1'
...
if (-not $szaFwdTarget) {
    ...
    if ($szaFwdCheckout) {
        ...
        $szaFwdProbe = Join-Path $szaFwdCandidates[-1] 'spec_catalog/check-probe-absent.ps1'
        if (Test-Path -LiteralPath $szaFwdProbe) { $szaFwdTarget = $szaFwdProbe }
    }
}
if (-not $szaFwdTarget) {
    Write-Host "check-probe-absent.ps1: the SZA harness is not installed - looked in:" -ForegroundColor Red
