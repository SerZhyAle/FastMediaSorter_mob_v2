...
<#
.SYNOPSIS
    Forwarder to the canon-shipped harness script all_features\validate.ps1 (S2402).
...
Exit codes: whatever all_features\validate.ps1 returns, plus 2 when the harness cannot be located.
#>
...
foreach ($szaFwdDir in $szaFwdCandidates) {
    $szaFwdProbe = Join-Path $szaFwdDir 'all_features\validate.ps1'
...
if (-not $szaFwdTarget) {
    ...
    if ($szaFwdCheckout) {
        ...
        $szaFwdProbe = Join-Path $szaFwdCandidates[-1] 'all_features\validate.ps1'
