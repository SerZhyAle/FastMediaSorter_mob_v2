...
<#
.SYNOPSIS
    Forwarder to the canon-shipped harness script all_features\remove.ps1 (S2402).
...
Exit codes: whatever all_features\remove.ps1 returns, plus 2 when the harness cannot be located.
#>
...
foreach ($szaFwdDir in $szaFwdCandidates) {
    $szaFwdProbe = Join-Path $szaFwdDir 'all_features\remove.ps1'
...
if (-not $szaFwdTarget) {
    ...
    if ($szaFwdCheckout) {
        ...
        $szaFwdProbe = Join-Path $szaFwdCandidates[-1] 'all_features\remove.ps1'
        if (Test-Path -LiteralPath $szaFwdProbe) { $szaFwdTarget = $szaFwdProbe }
    }
}
if (-not $szaFwdTarget) {
    Write-Host "remove.ps1: the SZA harness is not installed - looked in:" -ForegroundColor Red
