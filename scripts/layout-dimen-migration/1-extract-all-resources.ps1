...
function Should-Extract {
    ...
    $skipAttributes = @('id', 'layout_constraintTop_toTopOf', 'layout_constraintBottom_toBottomOf',
        ...
        'layout_cons traintLeft_toLeftOf', 'layout_constraintRight_toRightOf',
...
foreach ($file in $layoutFiles) {
    ...
    if ($fileCount -gt 0) {
        Write-Host "    [✓] Extracted: $fileCount | Skipped: $fileSkipped | Total attrs: $($matches.Count)" -ForegroundColor Green
    }
    else {
        Write-Host "    [·] Extracted: 0 | Skipped: $fileSkipped | Total attrs: $($matches.Count)" -ForegroundColor Gray
    }
}
...
$statsSkippedByReason.GetEnumerator() | Sort-Object Value -Descending | ForEach-Object {
    ...
    Write-Host "  ${reason}: $($_.Value)" -ForegroundColor Gray
}
Write-Host ""
...
$summary = @{
    ...
    skippedByReason = $statsSkippedByReason
    byType          = $statsByType
