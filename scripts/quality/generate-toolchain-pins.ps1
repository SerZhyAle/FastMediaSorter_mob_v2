...
.DESCRIPTION
    ...
    files and the version catalog (gradle/libs.versions.toml) and renders
...
$toml = if (Test-Path $tomlPath) { Get-Content -LiteralPath $tomlPath -Raw } else { '' }
...
$room = Get-Capture $toml ('androidx-room-runtime\s*=\s*"' + $ver + '"')
$media3 = Get-Capture $toml ('androidx-media3-exoplayer\s*=\s*"' + $ver + '"')
$glide = Get-Capture $toml ('glide-glide\s*=\s*"' + $ver + '"')
