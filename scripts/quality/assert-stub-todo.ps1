...
if ($ChangedFiles) {
    ...
    Write-Host ("stub-todo [delta over changed files]: new occurrences {0}" -f $d.Growth)
    if ($Gate -and $d.Growth -gt 0) {
        foreach ($p in $d.PerFile) { if ($p.New -gt 0) { Write-Host ("  +{0} in {1}" -f $p.New, $p.Path) } }
        Write-Host "FAIL: new runtime stub introduced in the changed file(s). Implement the branch or remove it (do not ship TODO()/NotImplementedError)."
        exit 1
    }
    exit 0
}
