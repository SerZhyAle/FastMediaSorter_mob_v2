...
#   .\scripts\builders\install-vr-debug-to-device.ps1 -ApkPath C:\custom\path.apk
#   .\a.ps1 ivrd
...
$variant = 'debug'
$packageName = 'com.sza.fastmediasorter.vr.debug'

Write-Host "VR APK installer — DEBUG" -ForegroundColor Cyan
...
if (-not $ApkPath -or -not (Test-Path -Path $ApkPath)) {
    Write-Host "Error: VR debug APK not found." -ForegroundColor Red
    Write-Host "Expected directory: $projectRoot\app_v2\build\outputs\apk\vr\$variant" -ForegroundColor Red
    Write-Host "Build it first:  .\scripts\builders\build-vr-debug.ps1   (or: .\a.ps1 vrd)" -ForegroundColor Gray
    exit 1
}
...
Write-Host "  Menu -> Library -> Unknown Sources -> FastMediaSorter (VR debug) -> tap to launch" -ForegroundColor White
