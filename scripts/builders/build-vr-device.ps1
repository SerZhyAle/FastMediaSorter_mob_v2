# Build VR Debug APK and Install on Device (Quest via ADB)
...
Write-Host "Target: Quest headset via ADB" -ForegroundColor Magenta
...
# Wait for device (Quest connected via USB or ADB over Wi-Fi)
...
$logEntry = "$timestamp | vr-debug-device | $destName"
...
$logFile = "$logDir\logcat_vr_$timestamp.log"
