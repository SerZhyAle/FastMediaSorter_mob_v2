...
Write-Host "Building Lite Release APK (auto-versioned)..." -ForegroundColor Cyan
...
# Start the Gradle build process (Release with R8 optimizations)
& $gradlew :app_v2:assembleLiteRelease "-Pfms.versionCode=$versionCodeInt" "-Pfms.versionName=$versionName" "-Pchaquopy.enabled=false" --configuration-cache
...
$apkDir = Join-Path $projectRoot "app_v2\build\outputs\apk\lite\release"
...
Write-Host "Package name: com.sza.fastmediasorter.lite" -ForegroundColor Cyan
...
$destName = "FastMediaSorter_lite_release.apk"
...
$logEntry = "$timestamp | lite-release | $destName"
