...
# Start the Gradle build process (Release with R8 optimizations)
& $gradlew :app_v2:assembleLegacyRelease "-Pfms.versionCode=$versionCodeInt" "-Pfms.versionName=$versionName" "-Pchaquopy.enabled=false" --configuration-cache
...
$apkDir = Join-Path $projectRoot "app_v2\build\outputs\apk\legacy\release"
...
Write-Host "Package name: com.sza.fastmediasorter.legacy" -ForegroundColor Cyan
...
$destName = "FastMediaSorter_legacy_release.apk"
...
$logEntry = "$timestamp | legacy-release | $destName"
