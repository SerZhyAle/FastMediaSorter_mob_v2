...
- `/spec*`: spec lifecycle (`/spec`, `/spec-all`, `/spec-tech`, `/spec-update`, `/spec-dev`, `/spec-check`, `/spec-fix`, `/spec-arc`, `/spec-test-device`, `/spec-sweep`), plus four specialized specs: `/spec-next` (pick and drive the top ticket of the queue), `/spec-do` (the same picker with no endpoint), `/spec-draft` (park an out-of-scope finding, CLAUDE.md 3.1), `/spec-quiz` (unstick a spec by asking the owner only the blocking decisions).
