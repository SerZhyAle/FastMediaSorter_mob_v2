...
| **Lite** | Облегченная версия, только локальные файлы | Видео, Аудио, Фото (без облака, без документов) |
| **Photos** | Оптимизирована для работы с фотографиями + облако | Только изображения + облачное хранилище (без видео/аудио) |
| **Legacy** | Полный функционал для старых Android (6.0+) | Видео, Аудио, Фото, Документы, EPUB, Облачные сервисы |
...
- **Lite**: Меньший размер приложения, без облака/документов/анимаций — фокус на локальных видео/аудио/фото
- **Photos**: Лучше для пользователей, работающих только с изображениями — включает поддержку облачного резервного копирования
- **Legacy**: Идентичный набор функций со Standard, но поддерживает Android 6.0+ (API 23)
...
- 🎛️ **Профили ресурсов:** Быстрая настройка при создании ресурса (Аудиотека, Видеотека, Фотохранилище, Документы, Все файлы) — автоматически задаёт фильтры типов файлов и порядок сортировки по умолчанию.
- 🔤 **Метаданные: отображение и сортировка:** Аудиофайлы показывают `Исполнитель - Название • Длительность` в списке. Сортировка по исполнителю, названию, длительности или дате съёмки. Метаданные кэшируются вместе со списком файлов.
...
**🗺️ Карта документации:** [Полный список документов](docs/DOCS_MAP.md)
