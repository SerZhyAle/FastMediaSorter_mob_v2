...
- §11, accumulated state (S1914): a criterion resting on state that accumulates **outside** the test session - a schema upgrade, an import, a bank merge, playback history - names that state in its own wording. A precondition is observable state, so this does not soften the rule above it; it is not a new field either, and adding one would be a graph filled in formally. Use the form the corpus already uses: «Канал, **уже закреплённый и с непустой историей**, переживает апгрейд схемы». Reason: S1832 required pinned channels, order, history and a launcher cell to survive an upgrade, and the device carried none of them - every criterion passed by having nothing to lose, with a real log and a real green result. A criterion about surviving a restart or a rotation is exempt on purpose: the tester creates that state in a step of the same scenario, so the demand would be ritual. Gate: `scripts/quality/assert-acceptance-preconditions.ps1` (baselined; it refuses new criteria, not the existing corpus).
