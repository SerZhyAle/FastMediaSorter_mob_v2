...
5. If the task touches UI strings or dialogs, keep the copy compact enough for Android layouts. Edit strings via `pwsh -NoProfile -File scripts/utils/set-android-string.ps1` (byte-preserving): `-Action set -Locale en|ru|uk -Key -Value` for a single-locale wording fix (`-ExpectedOldValue` guards against editing the wrong text), `-Action add -En -Ru -Uk` to introduce a key across all three locales at once. After `strings.xml` changes run `pwsh -NoProfile -File scripts/check_strings_localized.ps1 -KeyPrefix "<key_prefix>"`.
