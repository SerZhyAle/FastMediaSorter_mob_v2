...
- Tap by identifier or label, not by a coordinate you read earlier: `adb.ps1 tap-id -ResourceId "<name>"` and `adb.ps1 tap-label -Label "<text or content-desc>"` each dump the tree and tap in one call, and exit 8 without tapping when the target is not on screen. A remembered coordinate goes stale the moment the list scrolls, and then `tap -X -Y` silently hits the next row instead - which is an observation you would report as if it were the intended one (S1847). Reach for `tap-id` first: the label is translated, so a call aimed at one works on the locale you dumped and finds nothing on any other device (S1879); use `tap-label` when the element carries no id. `adb.ps1 uidump` shows what is on screen with each node's identifier and tap point, and `uidump -Ids` also lists the nodes that carry an identifier and no label.
