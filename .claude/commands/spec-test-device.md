...
2. `mobile_list_elements_on_screen` -> resolve coordinates from element ids (NEVER hard-code coordinates from a previous run - densities and dynamic layouts shift them).
3. Perform action (`click`, `type_keys`, `swipe`, `press_button`, `open_url`).
...
6. Record one row in scenario `## Run log` table: `step | action | result (PASS/FAIL/INCONCLUSIVE) | evidence (screenshot path + log lines)`.
...
| Toast / Snackbar disappears before screenshot | 2-second auto-dismiss | Re-screenshot within 1 s of action; if still missed, mark `INCONCLUSIVE`, rely on logcat trace |
