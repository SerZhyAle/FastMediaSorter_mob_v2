...
**Play FGS gate is NOT a hard blocker.** `publish-play-release.ps1` uploads the AAB fine but the COMMIT returns HTTP 403 (`You must let us know whether your app uses any Foreground Service permissions`) whenever the FGS declaration needs (re)confirming - owner-only Play Console action. Record it as `[PLAY FGS]` in the final report (listing the declared `FOREGROUND_SERVICE_*` types from the merged manifests; MEDIA_PROJECTION / MICROPHONE / camera / location need a short demo-video link) and continue the pipeline - the owner re-runs the publisher after saving the declaration; the uploaded AAB is reused.
