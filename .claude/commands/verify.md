...
Lightweight on-device smoke verification of a code change or behaviour claim. Builds (optional), installs (optional), launches the app, walks a minimal UI scenario via mobile-mcp, captures logcat, and reports PASS/FAIL with evidence. Does **not** touch `PLAN/`, `dev/CHANGELOG.md`, or `PLAN/spec-catalog.jsonl`. Use this when you want quick "does it actually work on a device" without the bureaucracy of `/spec-test-device`.
...
/verify <Sxxxx>                                   # read spec §11 criteria, treat as scenario hints; do NOT patch the spec
...
/verify --device <id>                             # pick adb id when multiple are online
/verify --release                                 # use release variant (avoid LeakCanary launcher on debug)
/verify --dry-run                                 # author the scenario only, no device execution
```

Hard requirement: the **mobile-mcp** server must be reachable. If `mcp__mobile-mcp__mobile_list_available_devices` is not available, abort with `mobile-mcp not configured - ensure .mcp.json is present and Node.js / npx are installed`.
...
`<pkg>`:
- debug variants: `com.sza.fastmediasorter.debug`
- release variants: `com.sza.fastmediasorter`
...
Use the builder matching the flavor:
...
On build failure: capture last 80 lines of the builder output into `temp/verify_<TS>.md` `## Build log` and abort. Do not proceed.

Skip this step when `--build` is not set. The pre-flight will have already validated the package is installed (or not, in which case ask user whether to rebuild).
...
1. Free-text from the user -> distil into 1..5 steps.
2. `Sxxxx §11 Criteria` -> map each criterion to one minimal interactive step (no `## Last Audit` editing). Skip criteria that are infeasible without external fixtures (mark `out-of-scope`).
...
3. Perform the action (click / type / swipe / press button / open_url).
4. `mobile_take_screenshot` -> `step_NN_after.png`
5. Verify expected post-state.
6. Append one row to scenario's `## Run log` table: `step | action | result | evidence`.
...
If the scenario was built from an `Sxxxx` that is currently in `BlockNeedUserTest`, additionally grep for the spec's debug verification tags:

```powershell
.\scripts\utils\search-log.ps1 -LogFile "temp/verify_run_<TS>.log" -Pattern "<Sxxxx>:" -AppOnly
```

Each tag hit at `D/` level is "code path exercised". `I`/`W`/`E` lines containing the ticket id are instrumentation bugs (CLAUDE.md "Persistent log lines must not contain Sxxxx") - report them, but do not count as PASS evidence.

Append `## Log findings` to the scenario file: counts per level, top 3 errors with line refs, any exception block.
...
- **Read-only on the journal.** Never call `update.ps1`, `complete.ps1`, `archive.ps1`. The journal `updated` timestamp does **not** move for `/verify`.
- **Read-only on specs.** Never touch `PLAN/Sxxxx_*.md`. `## Last Audit`, `## Revision History`, `[manual]` boxes are owned by `/spec-test-device` / `/spec-check`.
...
- **Never read full logcat into context** when > 2 MB - use `search-log.ps1` and quote line numbers.
- **Never hardcode element coordinates** - re-list elements before each click.
- **Read-only zones** `V1/`, `v2_6/`, `spec_v2/`, `dev/archive/` are ignored entirely.

---

## Known device traps

(Inherits from `/spec-test-device` - the same LeakCanary launcher trap, density-shifting coordinates, share-sheet missing-FMS-entry, Snackbar disappearance. See `/spec-test-device` §"Known device traps".)
