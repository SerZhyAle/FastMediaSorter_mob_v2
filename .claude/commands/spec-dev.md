...
- **Debug verification tags belong only to `BlockNeedUserTest`.** Insert `Timber.d("Sxxxx: …")` tags only at the moment this skill flips the ticket into `BlockNeedUserTest` (after all phases done, when on-device verification is part of acceptance) — never at `Implemented`, never per-step. Never remove tags from this skill — that is `/spec-check`'s job on the `Verified` transition (or `/spec-update`'s on a re-open). See CLAUDE.md "Debug Verification Tags".
