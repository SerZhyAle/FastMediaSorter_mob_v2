...
**How to apply:** Read the injected time from context and use it as the `[HH:MM:SS]` prefix on the chat reply for the doc-writing step. If context shows no time (first session before hook fired), simply omit the prefix - never guess, never shell out to `date` just to look busy.
