...
**How to apply:** Whenever a docs-task closure chains multiple PowerShell helpers (string audit + dev-log + functionality-log), pick option 1 or 2 above. If a chain you launched returns suspiciously little output - especially "only the first script's log line, nothing after" - assume the dollar-escape trap fired, switch to newline-separated form, and rerun. Never paper over the silence by re-invoking only the missing steps; the diagnosis matters more than the patch.
