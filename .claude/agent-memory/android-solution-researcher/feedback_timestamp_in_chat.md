...
**How to apply:** Read the injected time from context and use it in `[HH:MM:SS]` prefix when bracketing report sections or status messages. If context shows no time (e.g. first session before hook fired), omit the prefix rather than guessing - never shell out to `date` from the research agent.
