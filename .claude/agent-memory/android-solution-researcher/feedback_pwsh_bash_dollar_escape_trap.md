...
The symptom is exactly what was hit in S0268 F2: chained `update.ps1 -Status Tactical; ./add_to_dev_log.ps1 ...; ./add_to_dev_log.ps1 ...; ..` produced only the first script's output, with no error message and no dev log entries from the later calls.
...
The research agent rarely chains scripts, but when it does (e.g. multiple `query.ps1` invocations) use one of these patterns:

1. **Best:** newline-separated `-Command` with no script block, no `if`/`exit` between calls. Each script call is one line. PowerShell's `-Command` accepts multi-line strings via the bash `"..."` form (newlines are preserved). Example:

   ```
   "/c/Program Files/PowerShell/7/pwsh.exe" -NoProfile -Command "
   ./dev/CATALOG/scripts/query.ps1 -ClassMatches '*Xr*'
   ./dev/CATALOG/scripts/query.ps1 -ClassMatches '*Vr*'
   ./dev/CATALOG/scripts/query.ps1 -Role 'Repository'
   "
   ```
