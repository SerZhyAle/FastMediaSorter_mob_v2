...
- When a `BuildConfig.<flavor-flag>` reference is found inside `src/main/java/`, do NOT describe it as the intended design in the report. Cite it under "Risks Identified" with severity Low (existing tech debt) and note that CLAUDE.md Rule 14 forbids new instances.
