...
- For runtime XR availability checks inside a class that compiles for all flavors, inject `XrDetectionFacade` and call `observeState()` / `XrEnvironmentDetector.detect()` (returns `NONE` / `VR_QUEST` / `ANDROID_XR`). Do NOT read `BuildConfig.SUPPORT_VR_PLAYER` from `src/main/` (Rule 15) - the facade is the contract.
