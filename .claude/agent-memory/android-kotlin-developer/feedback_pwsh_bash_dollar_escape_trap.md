---
name: feedback_pwsh_bash_dollar_escape_trap
...
**Why:** bash double-quote variable expansion happens before PowerShell ever sees the string, and the escape sequence `\$` only means "literal $" in PowerShell parsing - it has no effect on bash. The combination `bash double-quote + PowerShell script block + escaped dollar` collapses to malformed input. [[feedback_pwsh_efficiency]] already mandates `-NoProfile` and batching, but its examples are run from PowerShell itself (where `$LASTEXITCODE` works as written); the bash-as-launcher case needs different quoting.

**How to apply:**

When chaining multiple PowerShell scripts from a Bash-tool call (e.g. catalog sync + dev-log + spec-catalog update in one tool invocation):
