...
- If a subagent reports "an external editor / IDE is reverting my files," first suspect a concurrent agent's git/stash, not a real IDE - verify the actual on-disk state with `git status` + grep for the expected sentinels before re-doing work.
