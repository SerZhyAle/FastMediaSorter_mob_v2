...
2. **Never hand-roll the adb path** like `$adb="$env:LOCALAPPDATA/Android/Sdk/platform-tools/adb.exe"` (~600 hits). Always `scripts/devtest/adb.ps1 <verb>` or `.\a.ps1 adb <verb>` - it auto-discovers adb (not on PATH) with stable exit codes. See [[reference_adb_swiss_army]].
...
Same class, already covered by their own entries (confirmed high-volume too): echo/flush probes -> [[feedback_no_flush_echo_commands]] (701 hits); bash `grep`/`rg` instead of the Grep tool -> [[feedback_rg_gitignore_catalog]] (1040 hits); general wrappers-first -> [[feedback_cli_project_wrappers_first]] and [[feedback_check_existing_tooling]].
...
**How to apply:** before issuing any Bash/PowerShell call, check I am not (a) `cd`-prefixing, (b) rebuilding a path a wrapper already knows (adb especially), (c) re-probing what a `*-ready` script does, or (d) calling `next-id.ps1` when `insert.ps1` would allocate. To recall a script's parameters, use `scripts/utils/help.ps1 -Name <script>` instead of re-reading the file. See [[reference_script_help_cheatsheet]].
