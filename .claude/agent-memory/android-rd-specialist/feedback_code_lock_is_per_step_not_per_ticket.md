...
- **Release only with `exit-code-lock.ps1`.** `agent-lock.ps1 -Verb Release -Name Code` prints nothing, exits **0**, and leaves the lock held - the success-looking exit is the trap. Confirmed 2026-08-07 on S1478: two "releases" reported clean while `lock-status.ps1 -Name Code` still showed my own pid holding it. Always verify with `lock-status.ps1` after releasing, and note that `exit-code-lock.ps1` refuses (correctly) when the lock has since passed to another session.
