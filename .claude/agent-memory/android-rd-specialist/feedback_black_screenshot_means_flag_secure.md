...
**How to apply:** `scripts/devtest/adb.ps1 shot` now detects this itself and prints the explanation next to the file it wrote (`-Json` carries `secureWindow`), added by S1506 - so on a black PNG, read the tool's output first. The manual tell is unchanged: a black capture combined with a healthy `uiautomator dump` - correct text and bounds, taps at those coordinates work - plus the status bar and system dialogs appearing normally in the same frame, because those are separate unprotected windows. A healthy tree next to a black frame **confirms** FLAG_SECURE, it does not contradict it.
