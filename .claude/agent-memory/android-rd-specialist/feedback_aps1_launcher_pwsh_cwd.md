...
**Why:** both produce a green exit code with no `BUILD SUCCESSFUL`, so the notification's "exit 0" is meaningless (see [[background-task-exit-code-is-echo]]). I trusted it twice in one session and thought the wiring was validated when nothing had compiled.
