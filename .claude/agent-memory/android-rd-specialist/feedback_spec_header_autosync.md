...
- The journal stays the single source of truth: still resolve any `Sxxxx` status via `select.ps1`, never by reading the header (see [[feedback-verify-spec-id-before-pipeline]]).
