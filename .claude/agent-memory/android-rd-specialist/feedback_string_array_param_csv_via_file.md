...
**The wrong verdict can echo your own argument back, which is the nastiest form.** 2026-08-19, `collect-stream-candidates.ps1 -CatalogOnly -DeepSignal -PruneStatuses dead,unknown,geo` through `pwsh -File`: the sweep classified 318 `dead`, 1606 `unknown` and 77 `geo`, then printed `Nothing to prune (no rows classified: dead,unknown,geo)` and exited 0. The list in that sentence is `$PruneStatuses -join ', '`, so a single-element array holding the whole CSV renders identically to a correct three-element one - the message is proof of nothing. Cost one full 19534-URL sweep. The script now splits on comma internally (option b), so the CSV form is the correct one there from now on; the general rule is to distrust any "found nothing" that names a `[string[]]` argument in its own text, and to cross-check against a count the script printed earlier in the same run.
