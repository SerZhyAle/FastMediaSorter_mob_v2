...
**How to apply:** First action on an In-Progress continuation = reconcile against the live files. Grep the spec's headline symbols; check the expected new classes/tests exist on disk. Then mark phase steps to match reality, fill only the genuine gaps, and document any divergence in the Step Log. Related: [[subagent-impl-skips-final-phase]].
