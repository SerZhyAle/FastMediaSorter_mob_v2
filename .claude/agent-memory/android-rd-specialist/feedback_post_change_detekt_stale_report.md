...
- Genuinely pre-existing same-file debt surfaced by proximity (e.g. a Room `@Query` with one bound param per column tripping `LongParameterList`) is fixed with a one-line `@Suppress("LongParameterList")` + WHY comment - safe when the method is NOT already baselined (a baselined method would shift its signature, see [[feedback_detekt_baseline_signature_resurface]]).
