...
**TWO CAPTURE BACKENDS - mirror capture-flow changes in BOTH (S1042 lesson, 2026-07-13):** the actual capture+save+dispatch runs through TWO independent services, one per flavor path:
- **standard:** `ScreenCaptureService` (MediaProjection, `src/screenCapture`) - `processCapture()` copies clipboard, saves via `SaveScreenshotUseCase`, then `actionDispatcher.runPostSave(action, savedUri)`.
- **noLegal:** `ScreenshotAccessibilityService` (accessibility, `src/noLegal`) - `saveBitmap()` does the SAME sequence and also calls `actionDispatcher.runPostSave(...)`.
Both share the `src/main` dispatcher (`handlePreCaptureAction` pre-capture, `runPostSave` post-save). So any change to the CAPTURE FLOW - intercepting/rerouting a gesture action around the save (e.g. S1042 routing OCR_TRANSLATE into a temp-staged crop screen instead of saving) - MUST be applied in BOTH services, not just one. Editing only `ScreenCaptureService` silently breaks noLegal (the dispatcher branch becomes a no-op there). **The owner device-tests on noLegal** (their real Galaxy S21+ runs `*-NoLegal-DEBUG`), so noLegal is the path that must work; check the log's `=== App version: ...-NoLegal-DEBUG ===` banner to confirm which flavor is installed before diagnosing.
