...
`scripts/devtest/adb.ps1 <verb>` (alias `.\a.ps1 adb <verb>`, shortcuts `adb-devices/-shot/-log/-current/-launch/-logcat-clear`) is the ad-hoc device swiss-army for quick manual work against a connected emulator/real device. Runs natively (~0 LLM tokens); auto-discovers adb (not on PATH); takes `-DeviceId`/`-Release`/`-Package`/`-OutDir`/`-Json`; stable exit codes (0 ok / 1 no-adb-or-bad-args / 2 no-device / 3 multi-device / 4 pkg-not-installed / 5 destructive-verb-refused / 6 pull-no-such-remote / 7 adb-failed / 8 tap-label-or-tap-id-target-absent / 9 clip-check-off-glass). Mirrors `device-ready.ps1` conventions.

Verbs: devices, props, current, launch (debug: explicit MainActivity, dodges LeakCanary), stop, logcat-clear (alias log-clear), wipe-data (needs `-Yes`), install (`-Apk`/`-Flavor`), uninstall (needs `-Yes`), shot (-> temp/), log (`-Tail N -Grep <regex>`, full dump -> temp/), tap (`-X -Y`), swipe (`-X -Y -X2 -Y2 [-Duration]`), text (`-Text`), key (`-Key`), prefs (run-as app_settings.xml), pull/push, shell (`-Cmd`), and the UI-tree verbs added by S1847 and S1879: uidump (`-Grep <regex>` matching label OR resource-id, `-Ids` to also list the nodes named by an id alone - every node with its short id, bounds and tap point), **tap-id** (`-ResourceId <short-or-full> [-Exact] [-Index N]` - the PREFERRED way to aim a tap: a label is translated and an id is not, so a `tap-label` call passes only on the locale it was written on and returns 8 on every other device; default match is a case-insensitive substring of the full id, so `-Exact` is how you say `rowExport` must not hit `rowExportAll`), tap-label (`-Label <s> [-Exact] [-Index N]` - still right where there is no id, which is most of Compose on the watch; both tap verbs dump and tap in one call and exit 8 when the target is absent), clip-check (content leaving the display shape; the shape is read off the device via `mRoundedCorners`, so the round watch and the rounded-corner phone share one rule; exit 9 only on the OFF-GLASS class).
