...
- [Measure throughput first](feedback_measure_throughput_before_calling_a_plan_infeasible.md) - ~16-20/day
...
- [sza_resources.xml missing here](project_sza_resources_absent_in_this_checkout.md) - prerelease configure dies; app stays EN and Maestro RU selectors "fail"
...
- [Lock per step](feedback_code_lock_is_per_step_not_per_ticket.md) · [Never batch lock](feedback_never_batch_code_lock_with_the_edit.md) · [Lock denial](feedback_code_lock_denial_does_not_stop_the_batch.md)
...
- [Deleted resource still ships](project_stale_merged_resource_outlives_its_source.md) - orphan .flat wins
...
- [Build gotchas](project_build_gotchas.md) + [pipes hide exits](feedback_build_output_pipe_truncation.md) + [a.ps1](feedback_aps1_launcher_pwsh_cwd.md) · [Gradle via pwsh](feedback_gradle_via_powershell_not_bash.md)
...
- [`$stable` reflection](feedback_compose_stable_field_in_reflection_tests.md) · [Sandbox deps](feedback_sandbox_tests_carry_a_dependency_manifest.md) · [fk misses Hilt](feedback_fk_does_not_validate_hilt_graph.md)
...
- [Probe must be one line](feedback_debug_probe_must_be_one_line.md) - a wrapped Timber.d( hides the probe from the removal grep
- [Probe vs 120 chars](feedback_probe_tag_collides_with_detekt_line_length.md) - one line, under 120, never wrap
...
- [configChanges](project_streams_activity_config_changes_rotation.md) · [InputRow greedy width](feedback_settingsinputrow_greedy_width.md) · [Canonical pickers](feedback_canonical_settings_value_pickers.md)
...
- [Dedup search is one token vs the slug](feedback_spec_search_matches_slug_tokens_only.md) - a multi-word query says "(no records)" and reads as "no duplicate"
...
- [Lease before research, not before edit](feedback_claim_ticket_lease_before_research_not_before_edit.md) - a sibling can close the whole ticket while you read; free locks prove nothing
...
- [Only you install on a device](feedback_orchestrator_owns_device_installs.md) - agent swapped the owner's flavor; `-d` hides a downgrade
