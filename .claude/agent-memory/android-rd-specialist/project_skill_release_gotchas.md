...
**9. S0386 "six flavors build-green" was DEBUG-green; the release bundle (R8 on) exposed two translate_feature DFM defects.**
- The translate_feature dynamic-feature module is code-less + resource-less (pure on-demand container; ML Kit ships bundled in app_v2 base via `standardImplementation` + `src/translationMlKit`). On RELEASE: (a) R8 strips it to 0 dex -> `packageStandardReleaseBundle` fails "no dex files but hasCode is not false" (fix: `<application android:hasCode="false"/>` in its manifest); (b) `:translate_feature:bundleLegacyReleaseResources` fails `AAPT invalid package ID 0x7e` because DFM `minSdk=26` vs legacy flavor `minSdk=23`.
- App gates translation availability on `SplitInstallManager.installedModules.contains("translate_feature")` and downloads via `RealDeliverableSetDownloader.downloadDfm("translate_feature")`, so you CANNOT just delete the DFM without reworking the gate. Parked as S0423.
- **Why:** R8/bundletool only run on release; DFM splits are never validated by debug builds. The whole on-demand-translation-for-store path (comment claims it, but `standardImplementation` still bundles ML Kit) is half-finished.
- **How to apply:** never trust "flavors build-green" for a DFM/R8 feature unless it was a RELEASE bundle build. For 2026-06-14 plateau, GitHub spectrum was deferred (Play standard shipped alone) until S0423 fixes the legacy DFM build.
