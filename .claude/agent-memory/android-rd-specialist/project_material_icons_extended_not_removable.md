...
**How to apply:** Treat any "material-icons-extended is replaceable by core" claim as unverified until a build proves it - the core set is a small curated subset and most named icons require extended. To actually drop extended you must first migrate Pause/SkipNext/SkipPrevious to vector drawables or different core icons (a UI task, not a dead-dep removal). In release, R8 already strips unused extended icons, so the AAB-size argument for removal is weak. Reinforces [[feedback_verify_subagent_build_failures]]: confirm subagent dependency-removal verdicts with an actual build.
