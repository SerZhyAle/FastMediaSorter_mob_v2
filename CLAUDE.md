...
- **Release plan - two files, split by "is there work left".** `PLAN/RELEASE_QUEUE.md` = what is still LEFT TO DO, in the owner's hand-kept order: every status below `Implemented`, including `BlockByOtherTask` / `BlockQuestions` / `BlockExternal` (blocked work still needs sorting). `PLAN/RELEASE_READY.md` = the release's finished content: `Implemented`, `Verified`, **and `BlockNeedUserTest`** - a device check that keeps not happening still ships; if it later proves broken it returns as fresh work in a later package. Column `rel` = release package number (matches the `DEBUG-v0NN` branch), `--` = not scheduled.
- **The queue decides what gets worked on next - always recommend from it, and obey it.** Any answer to "what next" (`/spec-next`, `/spec-all` with no id, a plain-language ask, a plan you propose) is ordered by `PLAN/RELEASE_QUEUE.md`: release package ascending, then the owner's line order inside that package. Buckets after the numbered packages, in order: `Implemented` rows in `PLAN/RELEASE_READY.md` (finished content awaiting its audit), then queue lines parked at `--`, then anything in neither file. Catalog `priority` is a tiebreak for unlisted tickets only - never the primary key. Quote the package and line you are acting on; a deviation needs a stated reason, and a silent one is a defect. Mechanically enforced in `scripts/spec_catalog/spec-next-preflight.ps1` and `release-plan.ps1`, which both rank from that file. A ticket you cannot place there is not ready to be started.
- Catalog owns STATUS; the two files own RELEASE ASSIGNMENT + ORDER. Every catalog write reconciles both automatically (`Sync-ReleaseQueue` in `_lib.ps1`) and moves a ticket across when it crosses the ready boundary - in either direction. No skill calls this directly. **A script may add, refresh a status/date, move a line between the two files, or drop an archived one - it must never reorder lines or touch the `rel` column.** Operator CLI: `scripts/spec_catalog/release-queue.ps1` (`-Reconcile` / `-Validate` / `-List [-Ready]` / `-SetCurrent` / `-Ship`). `-Ship` moves the READY block to `PLAN/RELEASE_QUEUE_DONE.md` and reports queue leftovers, which the owner re-sorts into a later package.
...
- **Background long jobs**: Launch every gradle-backed `a.ps1` target with `run_in_background: true` from the start - not just full/release builds but the compile-checks too (`d/db/dav/cd/nd/nl/r/fk/fkn/fc/fu`). A cold gradle daemon routinely exceeds the 120s foreground Bash timeout, and a forced-background handoff loses clean output capture and wastes a turn. The harness re-invokes on completion; continue other work meanwhile. Never foreground-wait a build or compile-check. Still never run >1 gradle build at once - enforced by `temp/BUILD.LOCK` (Rule 23), not just convention.
...
24. Bash `find` safety: the rule itself has one home, canon `rules/GITHUB_INTERACTION.md` section 6. This repo owns only the enforcement, which is the delta worth writing down: the global PreToolUse hook `~/.claude/hooks/guard-find-command.ps1` blocks the tool call before bash spawns (exit 2), and file/class lookup goes through the `Glob`/`Grep` tools or `dev/CATALOG/scripts/query.ps1` - raw `find` is almost never needed here. This is a hook, not a mechanical `assert-*` gate - editing project scripts does not change it.
