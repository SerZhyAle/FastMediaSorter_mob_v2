# Стратегическая спецификация: S0695 - Долгое нажатие на трансляцию закрепляет/открепляет
...
**Tactical spec:** `PLAN/S0695_stream-longpress-pin-toggle/` (будет создан через `/spec-tech`)
...
долгое ажатие на плитку или строку трансляции . вместо вызывыать диалог удаления (который реализован в меню три кнопки) пусть вызывает фиксацию/закрепление ии отфиксацию/открепление
...
Следующий шаг: `/spec-tech S0695` - создаст `PLAN/S0695_stream-longpress-pin-toggle/` с фазами.
