# Стратегическая спецификация: S0694 - Полноэкранное открытие плеера видеотрансляций
...
**Tactical spec:** `PLAN/S0694_stream-player-fullscreen-controls/` (будет создан через `/spec-tech`)
...
Открытие видеотрансляции из списка (таблицы) - открывается видеоплеер. Открывать сразу на полный экран. При нажатии на экран видеотрансляции должен появяться контроль управлени видеоплеером (выезжающая панель снизу, которая есть) и появляться командная панель сверху. Где есть клавища на полный экран, если пользователь захочет к нему вернуться. (9 или 3 тач-зон для плера трансляций не нужны.
...
Следующий шаг: `/spec-tech S0694` - создаст `PLAN/S0694_stream-player-fullscreen-controls/` с фазами.
