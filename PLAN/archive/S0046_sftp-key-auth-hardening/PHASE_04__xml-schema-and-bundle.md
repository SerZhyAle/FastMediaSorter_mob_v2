...
- 2026-06-10 — Verification 3/3 PASS. Files: assets/sftp_keys/.gitkeep (new), sza_resources.xml (header documents auth/privateKeyAsset/keyPassphrase/hostKeyFingerprint). Dev log recorded.
...
- 2026-06-10 — Verification 6/6 PASS. Files: SzaResourcesImporter.kt (new, @Inject @Singleton; constructor adds getDestinationsUseCase beyond prompt; import(): ImportResult; bundled-key read via assets, skip-on-unreadable, fingerprint canonicalization, no key/passphrase logging). Prompt deviations: actual factory param is sshPrivateKey (not plaintextPrivateKey); permanent logs carry no S0046 prefix; keyPassphrase warned as non-persistable (no schema column). Dev log recorded.
...
- 2026-06-10 — Verification 3/3 PASS. XmlPullParser 0 hits; szaResourcesImporter.import() once; LOC 792 -> 618. Constructor param + import added; body delegates to importer with toast/Timber.e on result. Dev log recorded.
...
- 2026-06-10 — Verification 2/2 PASS. `@Inject constructor` present in SzaResourcesImporter; all deps Hilt-provided (no module change). assembleStandardDebug BUILD SUCCESSFUL (Hilt graph resolved). Dev log recorded.
...
- [x] Project compiles — `assembleStandardDebug` BUILD SUCCESSFUL (2m59s, 2026-06-10).
