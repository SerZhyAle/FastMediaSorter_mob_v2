# Стратегическая спецификация: S0803 - Мини-игра, изменение размера поля по номеру уровня
...
**Tactical spec:** `PLAN/S0803_mini-game-level-field-scaling/` (будет создан через `/spec-tech`)
...
минигра. правила формирования новых уровне
изменения в размере поля
первый уроверь -  8 на 8
дальше формула размера такая
8 + ([номер уровня]/3)
не округлённое , а с отброшенной ддробной частью
но не больше 20 на 20
...
Следующий шаг: `/spec-tech S0803` - создаст `PLAN/S0803_mini-game-level-field-scaling/` с фазами.
