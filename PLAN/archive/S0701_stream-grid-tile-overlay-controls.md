# Стратегическая спецификация: S0701 - Оверлеи плитки в сетке трансляций (индикатор статуса + меню три точки)
...
**Tactical spec:** `PLAN/S0701_stream-grid-tile-overlay-controls/` (будет создан через `/spec-tech`)
...
в сетке трансляций потерялась индикация статуса зеленый-никакой(желтый)-красный про трансляции. Нужно маленький шарик индикации в левый нижний угол миниатюры поместить
второе потерялась кнопка "три точки" с командами над трансляцией (как в списке) - её нужно разместить в правом ерхнем углу миниатюры
...
Следующий шаг: `/spec-tech S0701` - создаст `PLAN/S0701_stream-grid-tile-overlay-controls/` с фазами.
