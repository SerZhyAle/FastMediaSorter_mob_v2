# Стратегическая спецификация: S0696 - Фильтр трансляций "только закреплённые"
...
**Tactical spec:** `PLAN/S0696_stream-filter-pinned-only/` (будет создан через `/spec-tech`)
...
добавить в фильтр трансляций опчию "только закреплфные", то есть те которые пользователь уже закрепил лично
...
Следующий шаг: `/spec-tech S0696` - создаст `PLAN/S0696_stream-filter-pinned-only/` с фазами.
