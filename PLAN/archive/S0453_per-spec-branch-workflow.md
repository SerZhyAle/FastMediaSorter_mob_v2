# Стратегическая спецификация: S0453 - Ветка git на каждую спеку в скилах
...
**Tactical spec:** `PLAN/S0453_per-spec-branch-workflow/` (будет создан через `/spec-tech`)
...
изменить скилы таким образом, чтобы они создавали branch по имени каждой спеки над которой будут работать, выполняли работы коммитя в этот бранч и собирая тестовые сборки на этом бранче, а в конце работы - сами его мерджили с бранчами типа DEBUG=vXXX (при передаче на тестирование пользователя или архивировании) а Релиз собирал их все и сводил в одну точку во время мерджа с main
...
Следующий шаг: `/spec-tech S0453` - создаст `PLAN/S0453_per-spec-branch-workflow/` с фазами.
